...
if (isset($_POST['submit'])) {
    ...
    if ($type === 'application') {
        ...
        if ($status === 'approved' && $tenantEmail) {
    ...
    if ($tenantEmail) {
        ...
        try {
            ...
            try {
                ...
                if ($tenantMobile) {
                    ...
                    error_log("No mobile number found for tenant ID: $id");
                }
            } catch (Exception $e) {
        ...
        file_put_contents(__DIR__ . '/approval_log.txt', $logMsg, FILE_APPEND);
        error_log("⚠️⚠️⚠️ NO EMAIL ADDRESS FOUND for $type ID: $id - Email NOT sent!");
    }
    ...
    if (strtolower($status) === 'approved') {
        if ($tenantEmail) {
            ...
            if ($smsResult && $smsResult['success']) {
                ...
                $message .= " Email notification sent. SMS failed.";
            } else {
                $message .= " Email notification sent.";
            }
            $_SESSION['success'] = $message;
        } else {
            $smsStatus = ($smsResult && $smsResult['success']) ? "SMS sent" : "SMS failed";
            $_SESSION['warning'] = "⚠️ Application #$id has been <strong>APPROVED</strong>. Warning: No email address found. $smsStatus.";
        }
        ...
        if ($tenantEmail) {
            ...
            $smsStatus = ($smsResult && $smsResult['success']) ? "SMS sent" : "SMS failed";
            $_SESSION['warning'] = "⚠️ Application #$id has been <strong>DECLINED</strong>. Warning: No email address found. $smsStatus.";
        }
    }
