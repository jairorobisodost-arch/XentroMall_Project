    ...
    if ($billingMonthRaw !== '' && preg_match('/^\d{4}-\d{2}$/', $billingMonthRaw)) {
        ...
        try {
            ...
            foreach ($uploadKeys as $key) {
                ...
                // Fetch real stall number for descriptive filename
                $stmtStallNo = $pdo->prepare("SELECT stall_number FROM stalls WHERE id = ?");
                $stmtStallNo->execute([$stallId]);
                $stallNumber = $stmtStallNo->fetchColumn() ?: 'Unknown';
                
                $monthYear = date('F_Y', strtotime($billingMonth));
                $uploadDir = 'uploads/';
                ...
                if ($hasWaterFile) {
                    ...
                    $ext = pathinfo($_FILES['bill_image']['name'][$key], PATHINFO_EXTENSION);
                    $fileName = "WaterBill_" . $monthYear . "_Stall_" . $stallNumber . "_" . substr(uniqid(), -5) . "." . $ext;
                    $targetPath = $uploadDir . $fileName;
                    if (move_uploaded_file($tmpName, $targetPath)) {
                        $waterImagePath = $targetPath;
                        $debugLog[] = "Uploaded water photo: {$fileName}";
                    }
                }
                ...
                if ($hasElectricFile) {
                    ...
                    $ext = pathinfo($_FILES['electric_bill_image']['name'][$key], PATHINFO_EXTENSION);
                    $fileName = "ElectricBill_" . $monthYear . "_Stall_" . $stallNumber . "_" . substr(uniqid(), -5) . "." . $ext;
                    $targetPath = $uploadDir . $fileName;
                    if (move_uploaded_file($tmpName, $targetPath)) {
                        $electricImagePath = $targetPath;
                        $debugLog[] = "Uploaded electric photo: {$fileName}";
                    }
                }
...
<html lang="en">
 <head>
  ...
  <style>
    ...
    @keyframes unreadRowPulse {
      0% { background: linear-gradient(90deg, #f0fdf4 0%, #ffffff 100%); }
      100% { background: linear-gradient(90deg, #dcfce7 0%, #ffffff 100%); }
    }
    ...
    @media (max-width: 1024px) {
      /* Header adjustments */
      header .max-w-\[1400px\] { padding-left: 12px; padding-right: 12px; }
      ...
      #mobileMenuBtn:hover { background: rgba(255,255,255,0.25); }
      ...
      #adminInfoDesktop { display: none !important; }
    }
    ...
    @media (max-width: 768px) {
      ...
      /* Cards padding */
      .gradient-card { padding: 16px !important; }
      .gradient-card .text-3xl { font-size: 1.5rem !important; }
      .gradient-card .w-14 { width: 40px !important; height: 40px !important; }
      ...
      .table-modern { min-width: 700px; }

      /* Header date hide */
      #headerDateBox { display: none !important; }

      /* Floating action reposition */
      .floating-action { bottom: 1rem; right: 1rem; }
      ...
      div[id*="modal"] > div {
        width: 94% !important;
        max-width: 450px !important; /* Cap width for better look on small screens */
        max-height: 92vh !important;
        overflow-y: auto !important;
        padding: 0 !important; /* Let internal sections handle padding */
      ...
      @media (max-width: 480px) {
        .fixed.inset-0 > div, div[id*="Modal"] > div {
          width: 96% !important;
        }
      }

      /* Override excessive paddings */
      .fixed.inset-0 .px-8, div[id*="Modal"] .px-8 {
        padding-left: 1.25rem !important;
        padding-right: 1.25rem !important;
      }
      .fixed.inset-0 .py-6, div[id*="Modal"] .py-6 {
      ...
      .fixed.inset-0 .flex.gap-4, div[id*="Modal"] .flex.gap-4 {
        flex-direction: column !important;
        gap: 0.75rem !important;
      }

      .fixed.inset-0 .flex.gap-4 button, div[id*="Modal"] .flex.gap-4 button {
        width: 100% !important;
      ...
      .fixed.inset-0 h2, div[id*="Modal"] h2 {
    ...
    @media (max-width: 480px) {
      ...
      .admin-layout-wrapper { padding: 12px !important; }
      header .max-w-\[1400px\] { padding-left: 8px; padding-right: 8px; }
 ...
 <body class="min-h-screen">
    ...
    <div class="max-w-[1400px] mx-auto px-4 lg:px-6 py-2">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <div id="adminInfoDesktop" class="bg-white rounded-2xl px-4 py-2 flex items-center gap-3 shadow-lg border-2 border-emerald-100">
            <div class="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-blue-500 flex items-center justify-center shadow-md">
              <i class="fas fa-user-shield text-white text-base"></i>
            </div>
            <div class="flex flex-col">
              <span class="text-[10px] text-gray-500 font-medium uppercase tracking-wide">Logged in as Admin</span>
              <p class="font-bold text-gray-900 text-sm leading-tight"><?php echo htmlspecialchars($admin_username); ?></p>
            </div>
          </div>
          
          <div class="relative">
            <button aria-label="Toggle menu" class="glass-effect text-white hover:bg-white/20 transition-colors p-3 rounded-xl" id="adminSettingsToggle">
            ...
            <div id="adminSettingsMenu" class="hidden absolute right-0 top-14 bg-white rounded-2xl shadow-2xl w-56 z-50 overflow-hidden border border-gray-100">
              <a href="admin_settings.php" class="block px-4 py-3 text-gray-700 hover:bg-emerald-50 hover:text-emerald-700 transition-colors border-b border-gray-50">
  ...
  <div class="flex gap-6 max-w-[1400px] mx-auto px-4 lg:px-6 py-6 admin-layout-wrapper">
   <!-- Sidebar -->
   <aside id="adminSidebar" class="gradient-primary rounded-2xl w-72 p-4 flex flex-col gap-3 shadow-2xl text-white relative overflow-hidden">
    ...
    <!-- Mobile close button -->
    <button onclick="closeMobileSidebar()" class="lg:hidden absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all border border-white/20">
      <i class="fas fa-times text-lg"></i>
    </button>

    <!-- Navigation -->
    <nav class="flex flex-col gap-2 relative z-10">
      <a class="nav-item flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewDashboardLink">
      ...
      <div class="nav-group mb-1">
        ...
        <div class="nav-dropdown">
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewTenantsLink">
            <i class="fas fa-users text-lg"></i>
            <span class="font-semibold">Tenants</span>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewApplicationsLink">
            <i class="fas fa-file-alt text-lg"></i>
            <span class="font-semibold">Applications</span>
            <?php if ($pendingApplications > 0): ?>
              <span class="ml-auto bg-indigo-500 text-white text-xs font-bold px-2 py-1 rounded-full animate-pulse"><?php echo $pendingApplications; ?></span>
            <?php endif; ?>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all group" href="stall_page.php">
            <i class="fas fa-store text-lg"></i>
            <span class="font-semibold">Stall Management</span>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all group" href="admin_work_permits.php">
            <i class="fas fa-hard-hat text-lg"></i>
            <span class="font-semibold">Work Permits</span>
            <?php if ($pendingWorkPermits > 0): ?>
              <span class="ml-auto bg-indigo-500 text-white text-xs font-bold px-2 py-1 rounded-full animate-pulse"><?php echo $pendingWorkPermits; ?></span>
            <?php endif; ?>
          </a>
        </div>
      </div>
      ...
      <div class="nav-group mb-1">
        ...
        <div class="nav-dropdown">
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewRenewalLink">
            <i class="fas fa-sync-alt text-lg"></i>
            <span class="font-semibold">Renewal</span>
            <?php if ($pendingRenewals > 0): ?>
              <span class="ml-auto bg-indigo-500 text-white text-xs font-bold px-2 py-1 rounded-full animate-pulse"><?php echo $pendingRenewals; ?></span>
            <?php endif; ?>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewPaymentsLink">
            <i class="fas fa-credit-card text-lg"></i>
            <span class="font-semibold">Payments</span>
            <?php if ($pendingPayments > 0): ?>
              <span class="ml-auto bg-indigo-500 text-white text-xs font-bold px-2 py-1 rounded-full animate-pulse"><?php echo $pendingPayments; ?></span>
            <?php endif; ?>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewBillingLink">
            <i class="fas fa-file-invoice-dollar text-lg"></i>
            <span class="font-semibold">Billing</span>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewSOALink">
            <i class="fas fa-file-invoice text-lg"></i>
            <span class="font-semibold">Statement of Account</span>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewContractsLink">
      ...
      <div class="nav-group mb-1">
        ...
        <div class="nav-dropdown">
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewHistoryLink">
            <i class="fas fa-chart-bar text-lg"></i>
            <span class="font-semibold">Revenue Reports</span>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all cursor-pointer group" id="viewApplicantHistoryLink">
      ...
      <div class="nav-group mb-1">
        ...
        <div class="nav-dropdown">
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all group" href="posting.php">
            <i class="fas fa-bullhorn text-lg"></i>
            <span class="font-semibold">Announcements</span>
          </a>
          <a class="nav-item nav-item-sub flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/15 transition-all group" href="admin_register.php">
   ...
   <!-- Main content -->
   <main id="mainContent" class="flex-1 flex flex-col gap-6 overflow-auto scrollbar-custom" style="max-height: 90vh;">
    <!-- Success/Error Messages -->
    <?php if (isset($_SESSION['success'])): ?>
      <div class="bg-gradient-to-r from-emerald-50 to-green-50 border-l-4 border-emerald-500 text-emerald-700 px-6 py-4 rounded-2xl shadow-md animate-fade-in flex items-center gap-3 mb-6">
        <i class="fas fa-check-circle text-2xl flex-shrink-0"></i>
        <span class="font-medium text-lg"><?php echo $_SESSION['success']; unset($_SESSION['success']); ?></span>
      </div>
    <?php endif; ?>
    <?php if (isset($_SESSION['warning'])): ?>
      <div class="bg-gradient-to-r from-amber-50 to-orange-50 border-l-4 border-amber-500 text-amber-700 px-6 py-4 rounded-2xl shadow-md animate-fade-in flex items-center gap-3 mb-6">
        <i class="fas fa-exclamation-triangle text-2xl flex-shrink-0"></i>
        <span class="font-medium text-lg"><?php echo $_SESSION['warning']; unset($_SESSION['warning']); ?></span>
      </div>
    <?php endif; ?>
    <?php if (isset($_SESSION['error'])): ?>
      <div class="bg-gradient-to-r from-red-50 to-pink-50 border-l-4 border-red-500 text-red-700 px-6 py-4 rounded-2xl shadow-md animate-fade-in flex items-center gap-3 mb-6">
        <i class="fas fa-exclamation-circle text-2xl flex-shrink-0"></i>
        <span class="font-medium text-lg"><?php echo $_SESSION['error']; unset($_SESSION['error']); ?></span>
      </div>
    <?php endif; ?>
    ...
    <header class="animate-fade-in">
     <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-2 gap-3">
       <div>
         ...
         <p class="text-gray-600 mt-1 text-sm sm:text-base">Welcome back! Here's what's happening with your mall today.</p>
       </div>
    ...
    <section id="dashboardSection" class="space-y-6 animate-fade-in">
      ...
      <?php if ($newApplicationsCount > 0): ?>
        <div id="newAppAlert" class="relative overflow-hidden rounded-2xl shadow-lg mb-6 animate-fade-in" style="background: linear-gradient(135deg, #059669 0%, #0ea5e9 50%, #6366f1 100%);">
          <!-- Decorative elements -->
          ...
          <div class="relative z-10 p-5 sm:p-6">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div class="flex items-start gap-4">
                <div class="w-14 h-14 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg border border-white/20">
                  <i class="fas fa-file-circle-check text-white text-2xl"></i>
                </div>
                <div>
                  <div class="flex items-center gap-2 mb-1">
                    <h4 class="text-white font-bold text-lg">New Application<?php echo $newApplicationsCount > 1 ? 's' : ''; ?> Submitted!</h4>
                    <span class="bg-white/25 backdrop-blur-sm text-white text-xs font-black px-3 py-1 rounded-full border border-white/30 animate-pulse shadow-lg">
                  ...
                  <p class="text-white/90 text-sm">
                    <?php if ($newApplicationsCount == 1): ?>
                      ...
                      has submitted a new application<?php echo !empty($newApplicationsList[0]['stall_number']) ? ' for Stall #' . htmlspecialchars($newApplicationsList[0]['stall_number']) : ''; ?>.
                    <?php else: ?>
                      <strong><?php echo htmlspecialchars($newApplicationsList[0]['tradename'] ?? $newApplicationsList[0]['company_name'] ?? 'New Applicant'); ?></strong>
                      and <strong><?php echo ($newApplicationsCount - 1); ?> other<?php echo ($newApplicationsCount - 1) > 1 ? 's' : ''; ?></strong>
                      have submitted new applications.
                    <?php endif; ?>
                    &mdash; <span class="text-white/70">Submitted <?php echo date('M d, Y g:i A', strtotime($newApplicationsList[0]['created_at'])); ?></span>
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-2 w-full sm:w-auto flex-shrink-0">
                <button onclick="document.getElementById('viewApplicationsLink').click();" class="flex-1 sm:flex-none bg-white hover:bg-gray-50 text-emerald-700 px-5 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg flex items-center justify-center gap-2 border border-white/50">
                  <i class="fas fa-eye"></i> Review Now
                </button>
                <button onclick="dismissNewAppAlert()" class="w-10 h-10 flex items-center justify-center bg-white/15 hover:bg-white/25 text-white rounded-xl transition-all border border-white/20" title="Dismiss">
            ...
            <div class="mt-4 pt-4 border-t border-white/20">
              <div class="flex flex-wrap gap-2">
                <?php foreach ($newApplicationsList as $newApp): ?>
                  <div class="bg-white/15 backdrop-blur-sm rounded-lg px-3 py-1.5 text-white text-xs font-medium border border-white/10 flex items-center gap-2">
                    <i class="fas fa-user-circle"></i>
                    <?php echo htmlspecialchars($newApp['tradename'] ?? $newApp['company_name'] ?? 'Applicant'); ?>
                    <?php if (!empty($newApp['stall_number'])): ?>
                      <span class="bg-white/20 px-1.5 py-0.5 rounded text-[10px] font-black">Stall #<?php echo htmlspecialchars($newApp['stall_number']); ?></span>
                    <?php endif; ?>
                  </div>
                <?php endforeach; ?>
              </div>
      ...
      ?>
        <div class="grid grid-cols-1 gap-4 mb-6">
          <div class="bg-amber-50 border-l-4 border-amber-500 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between animate-fade-in ring-1 ring-amber-100 marquee-container gap-4">
            <div class="flex items-center gap-3 marquee-content w-full">
              <div class="flex items-center gap-3 shrink-0">
                ...
                <div class="flex-1">
                  <h4 class="text-amber-800 font-bold text-lg leading-tight">BIR Expiration Approaching</h4>
                  <p class="text-amber-700 text-sm"><?php echo $birUrgentCount; ?> tenant(s) have BIR registrations expiring within 30 days.</p>
                </div>
              </div>
            </div>
            <div class="flex flex-wrap items-center gap-2 relative z-10 shrink-0 w-full md:w-auto">
              <button onclick="viewTenantsLink.click()" class="flex-1 md:flex-none justify-center bg-amber-600 hover:bg-amber-700 text-white px-4 py-2.5 rounded-xl transition-all font-semibold shadow-md flex items-center gap-2 text-sm">
                <i class="fas fa-eye"></i> View Tenants
              </button>
              <button onclick="notifyAllExpiringBIR(event)" class="flex-1 md:flex-none justify-center bg-white hover:bg-gray-100 text-amber-700 border border-amber-200 px-4 py-2.5 rounded-xl transition-all font-bold shadow-sm flex items-center gap-2 text-sm">
      ...
      <!-- Recent Pending Payments Widget -->
      <?php if (!empty($recentPendingPayments)): ?>
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-6 animate-fade-in dashboard-widget">
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-bold text-gray-900 text-lg flex items-center gap-2">
              <i class="fas fa-credit-card text-emerald-500"></i> Recent Pending Payments
              <span class="bg-emerald-100 text-emerald-700 text-[10px] px-2 py-0.5 rounded-full uppercase font-black"><?php echo count($recentPendingPayments); ?> NEW</span>
            </h3>
            <button onclick="document.getElementById('viewPaymentsLink').click()" class="text-blue-600 hover:text-blue-700 font-bold text-xs flex items-center gap-1 transition-all">
              View All <i class="fas fa-arrow-right"></i>
            </button>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <?php foreach ($recentPendingPayments as $pay): ?>
              <div class="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 hover:bg-emerald-50/30 transition-all group">
                <div class="flex items-start justify-between mb-2">
                  ...
                  <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest"><?php echo date('M d, g:i A', strtotime($pay['payment_date'])); ?></span>
                </div>
                <p class="font-bold text-gray-900 leading-snug mb-1 truncate"><?php echo htmlspecialchars($pay['tenant_name']); ?></p>
                <div class="flex items-center gap-2 mb-3">
                  <span class="text-xs font-bold text-blue-600 px-2 py-0.5 bg-blue-50 rounded-lg">Stall <?php echo htmlspecialchars($pay['stall_number']); ?></span>
                  <span class="text-xs font-black text-emerald-600">₱<?php echo number_format($pay['amount'], 2); ?></span>
                </div>
                <div class="flex gap-2">
                  <?php if (!empty($pay['payment_image'])): ?>
                    <button onclick="openPaymentProofModal('<?php echo htmlspecialchars($pay['payment_image']); ?>')" class="flex-1 bg-white hover:bg-gray-100 text-gray-700 border border-gray-200 px-3 py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1">
                      <i class="fas fa-image"></i> Proof
                    </button>
                  <?php endif; ?>
                  <button onclick="document.getElementById('viewPaymentsLink').click()" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-xl text-xs font-bold transition-all shadow-sm">
                    Verify
                  </button>
                </div>
              </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div id="totalTenantsBox" class="cursor-pointer hover-lift gradient-card rounded-3xl p-6 shadow-lg border border-gray-100 relative overflow-hidden group">
        ...
        <div class="hover-lift gradient-card rounded-3xl p-6 shadow-lg border border-gray-100 relative overflow-hidden group">
        ...
        <div class="hover-lift gradient-card rounded-3xl p-6 shadow-lg border border-gray-100 relative overflow-hidden group">
          <div class="absolute -top-4 -right-4 w-24 h-24 bg-red-100 rounded-full opacity-50 group-hover:scale-110 transition-transform duration-500"></div>
          <div class="relative z-10">
            <div class="flex items-center justify-between mb-4">
        ...
        <div class="gradient-card rounded-3xl p-6 shadow-lg border border-gray-100">
          <div class="flex items-center justify-between mb-6">
            <div>
              ...
              <p class="text-gray-500 text-sm" id="paymentTrendsSubtitle">Daily payment collections</p>
            </div>
            <div class="flex items-center gap-4 -mt-1">
              <input type="month" id="paymentTrendsMonthSelector" value="<?php echo date('Y-m'); ?>" class="text-sm px-2 py-1 border border-gray-200 rounded-lg text-gray-600 focus:outline-none focus:ring-2 focus:ring-emerald-500" onchange="fetchPaymentTrends()">
          ...
          <div class="h-64 relative">
            <div id="paymentsBarLoading" class="absolute inset-0 bg-white/50 backdrop-blur-sm z-10 flex items-center justify-center hidden rounded-lg">
      ...
      <div class="gradient-card rounded-3xl p-6 shadow-lg border border-gray-100">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <button class="flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-emerald-50 hover:bg-emerald-100 transition-colors group text-center" id="quickApplications">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-emerald-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
              <i class="fas fa-file-alt text-white text-sm sm:text-base"></i>
            </div>
            <span class="text-[11px] sm:text-sm font-medium text-gray-700 leading-tight">View Applications</span>
          </button>
          <button class="flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-blue-50 hover:bg-blue-100 transition-colors group text-center" id="quickPayments">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-blue-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
              <i class="fas fa-credit-card text-white text-sm sm:text-base"></i>
            </div>
            <span class="text-[11px] sm:text-sm font-medium text-gray-700 leading-tight">Manage Payments</span>
          </button>
          <button class="flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-purple-50 hover:bg-purple-100 transition-colors group text-center" id="quickMaintenance">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-purple-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
              <i class="fas fa-tools text-white text-sm sm:text-base"></i>
            </div>
            <span class="text-[11px] sm:text-sm font-medium text-gray-700 leading-tight">Maintenance</span>
          </button>
          <button class="flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-amber-50 hover:bg-amber-100 transition-colors group text-center" id="quickBilling">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-amber-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
    ...
    <section id="tenantsSection" class="hidden space-y-6 animate-fade-in">
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="bg-gradient-to-br from-emerald-100 to-blue-100 text-emerald-700 px-5 py-3 rounded-2xl font-semibold shadow-md border border-emerald-200">
        ...
        <div class="grid grid-cols-1 gap-3">
          <?php foreach ($allTenants as $index => $tenant): ?>
            ...
            <div class="gradient-card rounded-2xl shadow-lg overflow-hidden border border-gray-100">
              ...
              <div class="p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between cursor-pointer hover:bg-gray-50 transition-colors gap-4" onclick="toggleTenantDetails(<?php echo $index; ?>)">
                <div class="flex items-center gap-4 w-full sm:w-auto">
                  <div class="w-12 h-12 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg shrink-0">
                    <?php echo strtoupper(substr($tenant['tradename'], 0, 2)); ?>
                  </div>
                  <div class="flex-1">
                    <h3 class="text-lg font-bold text-gray-900"><?php echo htmlspecialchars($tenant['tradename']); ?></h3>
                ...
                <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto sm:justify-end">
                  <span class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-semibold <?php echo $birBadgeClass; ?> border shadow-sm">
                    <i class="fas <?php echo $birIcon; ?> mr-1"></i><?php echo $birStatusText; ?>
                  </span>
                  <?php if ($tenant['stall_number']): ?>
                    <span class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-semibold bg-blue-100 text-blue-700 border border-blue-200">
                      <i class="fas fa-store mr-1"></i><?php echo htmlspecialchars($tenant['stall_number']); ?>
                    </span>
                  <?php endif; ?>
                  <button class="flex-1 sm:flex-none justify-center px-4 py-2 bg-gradient-to-r from-emerald-500 to-blue-500 text-white rounded-xl hover:from-emerald-600 hover:to-blue-600 transition-all font-medium shadow-md text-sm">
              ...
              <div id="tenant-details-<?php echo $index; ?>" class="hidden border-t border-gray-200 bg-gray-50 p-6">
                ...
                <div class="mb-6">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm <?php echo ($birStatus === 'expired') ? 'border-red-300 ring-2 ring-red-50' : ''; ?>">
                      <div class="flex items-center gap-3 mb-2">
                        <div class="w-8 h-8 <?php echo ($birStatus === 'expired') ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'; ?> rounded-lg flex items-center justify-center">
                          <i class="fas fa-file-invoice"></i>
                        </div>
                        <div class="flex-1">
                          <p class="text-xs text-gray-500 font-semibold uppercase">BIR Registration Expiry</p>
                          <p class="text-lg font-bold <?php echo ($birStatus === 'expired') ? 'text-red-600' : 'text-gray-900'; ?>">
                            <?php echo $birExpiry ? date('M d, Y', strtotime($birExpiry)) : 'Not Set'; ?>
                          </p>
                          <div class="flex flex-wrap items-center gap-y-1 mt-1">
                            <button onclick='openManualBIRUpdateModal(<?php echo json_encode(["id" => $tenant["id"], "tradename" => $tenant["tradename"], "expiry" => $tenant["bir_expiry_date"]]); ?>)' 
                            ...
                            <?php if ($birDocumentUrl): ?>
                              ...
                              <a href="<?php echo htmlspecialchars($birDocumentUrl); ?>" target="_blank"
                            ...
                            <?php if ($birStatus !== 'valid' && $birStatus !== 'not_set'): ?>
                              <span class="text-gray-300 mx-1">|</span>
                              <button onclick="notifyTenantBIR(event, <?php echo $tenant['id']; ?>, '<?php echo addslashes($tenant['tradename']); ?>')" 
                      ...
                      <?php if ($birExpiry): ?>
                        <p class="text-sm <?php echo ($birStatus === 'expired') ? 'text-red-500 font-bold' : 'text-gray-600'; ?>">
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <p class="text-xs text-gray-500 font-semibold uppercase">Stall Number</p>
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <p class="text-lg font-bold text-gray-900"><?php echo htmlspecialchars($tenant['business_type']); ?></p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <p class="text-lg font-bold text-gray-900"><?php echo htmlspecialchars($tenant['contact_person']); ?></p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <p class="text-lg font-bold text-gray-900"><?php echo htmlspecialchars($tenant['store_location']); ?></p>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div class="flex-1">
                          ...
                          <p class="text-lg font-bold text-gray-900"><?php echo htmlspecialchars($tenant['mobile']); ?></p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div class="flex-1">
                          ...
                          <p class="text-sm font-bold text-gray-900 break-all"><?php echo htmlspecialchars($tenant['email']); ?></p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-xl border-2 border-purple-200">
                      <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="flex-1">
                          <p class="text-xs text-purple-600 font-semibold uppercase">Contract Status</p>
                          <?php if ($tenant['lease_start_date'] && $tenant['lease_expiration_date']): ?>
                            ...
                            ?>
                            <p class="text-xl font-bold <?php echo $isExpired ? 'text-red-600' : 'text-purple-900'; ?>">
                      ...
                      <?php if ($tenant['lease_start_date'] && $tenant['lease_expiration_date']): ?>
                        <div class="space-y-2">
                          <div class="flex justify-between items-center">
                            <span class="text-sm text-purple-700"><i class="fas fa-calendar-check mr-1"></i>Start Date</span>
                            <span class="font-semibold text-purple-900"><?php echo date('M d, Y', strtotime($tenant['lease_start_date'])); ?></span>
                          </div>
                          <div class="flex justify-between items-center">
                            <span class="text-sm text-purple-700"><i class="fas fa-calendar-times mr-1"></i>End Date</span>
                            <span class="font-semibold text-purple-900"><?php echo date('M d, Y', strtotime($tenant['lease_expiration_date'])); ?></span>
                          </div>
                          <?php if (!$isExpired): ?>
                            <div class="mt-3 p-2 bg-purple-100 rounded-lg">
                              <p class="text-purple-900 font-bold text-center">
                                <i class="fas fa-hourglass-half mr-1"></i><?php echo $daysLeft; ?> days remaining
                              </p>
                            </div>
                          <?php endif; ?>
                        </div>
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-cyan-50 p-4 rounded-xl border-2 border-blue-200">
                      <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="flex-1">
                          <p class="text-xs text-blue-600 font-semibold uppercase">Latest Contract</p>
                          <?php $latestContract = $latestContractsByTenantDetail[$tenant['id']] ?? null; ?>
                          <?php if ($latestContract): ?>
                            <p class="text-lg font-bold text-blue-900"><?php echo strtoupper($latestContract['contract_status']); ?></p>
                            <p class="text-sm text-blue-700 mt-1">
                              Version <?php echo $latestContract['version']; ?> • <?php echo date('M d, Y', strtotime($latestContract['created_at'])); ?>
                            </p>
                          <?php else: ?>
                ...
                <div class="border-t border-gray-300 pt-6">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    <a href="tenant_profile.php?user_id=<?php echo $tenant['user_id']; ?>" class="flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl hover:from-blue-600 hover:to-purple-600 transition-all font-medium shadow-md hover:shadow-lg">
                      <i class="fas fa-store-alt"></i>
                      <span>View Profile</span>
                    </a>

                    <a href="generate_contract.php?tenant_detail_id=<?php echo $tenant['id']; ?>" class="flex items-center justify-center gap-2 px-4 py-3 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition-all font-medium shadow-md hover:shadow-lg">
                      <i class="fas fa-file-signature"></i>
                      <span>Generate Contract</span>
                    </a>

                    <?php if (!empty($latestContract)): ?>
                      <a href="view_contract.php?id=<?php echo $latestContract['id']; ?>" class="flex items-center justify-center gap-2 px-4 py-3 bg-slate-500 text-white rounded-xl hover:bg-slate-600 transition-all font-medium shadow-md hover:shadow-lg">
                        <i class="fas fa-eye"></i>
                        <span>View Contract</span>
                      </a>
                    <?php else: ?>
                      <button disabled class="flex items-center justify-center gap-2 px-4 py-3 bg-gray-300 text-gray-500 rounded-xl cursor-not-allowed font-medium">
                        <i class="fas fa-eye"></i>
                        <span>No Contract</span>
                      </button>
                    <?php endif; ?>

                    <button onclick="openEditContractModal(<?php echo $tenant['id']; ?>, '<?php echo addslashes(htmlspecialchars($tenant['tradename'])); ?>', '<?php echo addslashes($tenant['lease_start_date'] ?? ''); ?>', '<?php echo addslashes($tenant['lease_expiration_date'] ?? ''); ?>')" class="flex items-center justify-center gap-2 px-4 py-3 bg-purple-500 text-white rounded-xl hover:bg-purple-600 transition-all font-medium shadow-md hover:shadow-lg">
                  ...
                  <div class="mt-4 flex justify-center">
                    <button class="flex items-center justify-center gap-2 px-6 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all font-medium">
    ...
    <section id="applicationsSection" class="hidden space-y-6 animate-fade-in">
      ...
      <?php if (isset($_SESSION['success'])): ?>
        <div class="mb-4 rounded-2xl bg-gradient-to-r from-emerald-50 to-green-50 text-emerald-700 px-6 py-4 border-l-4 border-emerald-500 shadow-md flex items-center gap-3">
          <i class="fas fa-check-circle text-2xl"></i>
          <span class="font-medium"><?php echo htmlspecialchars($_SESSION['success']); unset($_SESSION['success']); ?></span>
        </div>
      <?php endif; ?>
      <?php if (isset($_SESSION['warning'])): ?>
        <div class="mb-4 rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 text-amber-700 px-6 py-4 border-l-4 border-amber-500 shadow-md flex items-center gap-3">
          <i class="fas fa-exclamation-triangle text-2xl"></i>
          <span class="font-medium"><?php echo htmlspecialchars($_SESSION['warning']); unset($_SESSION['warning']); ?></span>
        </div>
      <?php endif; ?>
      <?php if (isset($_SESSION['error'])): ?>
        <div class="mb-4 rounded-2xl bg-gradient-to-r from-red-50 to-pink-50 text-red-700 px-6 py-4 border-l-4 border-red-500 shadow-md flex items-center gap-3">
            ...
            <table class="w-full min-w-[800px]">
              <thead class="bg-gradient-to-r from-emerald-50 to-blue-50 border-b border-gray-200">
                <tr>
                  ...
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Business Info</th>
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Stall</th>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <?php foreach ($applications as $index => $app): ?>
                  ...
                  <tr class="hover:bg-gray-50 transition-colors<?php echo $isUnread ? ' unread-row' : ''; ?>" id="app-row-<?php echo $app['id']; ?>">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center">
                        <div class="flex-shrink-0 h-10 w-10">
                          <div class="h-10 w-10 rounded-full bg-gradient-to-br from-emerald-400 to-blue-500 flex items-center justify-center text-white font-bold text-sm">
                        ...
                        <div class="ml-4">
                          <div class="text-sm font-medium text-gray-900">
                            ...
                            <?php if (empty($app['admin_viewed'])): ?>
                              <span class="ml-2 unread-notif-badge" id="unread-badge-<?php echo $app['id']; ?>" title="New Application">
                                <span class="unread-dot"></span> NEW
                                <?php if ($app['approved_stalls_count'] > 0): ?>
                                  <span class="ml-1 text-[0.6rem] border-l border-red-300 pl-1">ADD'L</span>
                                <?php endif; ?>
                              </span>
                            <?php endif; ?>
                          </div>
                          <div class="text-sm text-gray-500">ID: <?php echo htmlspecialchars($app['user_id']); ?></div>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-900"><?php echo htmlspecialchars($app['company_name']); ?></div>
                      <div class="text-sm text-gray-500"><?php echo htmlspecialchars($app['business_type']); ?></div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <?php if ($app['stall_number']): ?>
                        <div class="text-sm text-gray-900"><?php echo htmlspecialchars($app['stall_number']); ?></div>
                        <div class="text-sm text-gray-500"><?php echo htmlspecialchars($app['stall_location']); ?></div>
                        <?php if ($app['monthly_rate']): ?>
                          <div class="text-xs text-emerald-600 font-semibold">₱<?php echo number_format($app['monthly_rate'], 2); ?>/mo</div>
                        <?php endif; ?>
                      <?php else: ?>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-900"><?php echo htmlspecialchars($app['mobile']); ?></div>
                      <div class="text-sm text-gray-500 truncate max-w-xs"><?php echo htmlspecialchars($app['email']); ?></div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <?php echo date('M d, Y', strtotime($app['created_at'])); ?>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                      <button onclick="openApplicationModal(<?php echo $index; ?>)" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition-all text-sm font-medium shadow-sm">
    ...
    <section id="renewalSection" class="hidden space-y-6 animate-fade-in">
              ...
              <div class="gradient-card rounded-2xl shadow-lg overflow-hidden hover-lift border border-gray-100">
                <div class="p-6">
                  <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                      <div class="flex items-center gap-3 mb-2">
                        <div class="w-12 h-12 bg-gradient-to-br from-purple-400 to-pink-500 rounded-xl flex items-center justify-center text-white font-bold text-lg">
                          <?php echo strtoupper(substr($renewal['tradename'], 0, 2)); ?>
                        </div>
                        <div>
                          <h3 class="text-xl font-bold text-gray-900"><?php echo htmlspecialchars($renewal['tradename']); ?></h3>
                          <p class="text-sm text-gray-500">
                            <i class="fas fa-store mr-1"></i>
                            Stall: <?php echo htmlspecialchars($renewal['stall_number'] ?? 'N/A'); ?>
                            <span class="mx-2">•</span>
                            <i class="fas fa-calendar mr-1"></i>
                            Submitted: <?php echo date('M d, Y', strtotime($renewal['submitted_at'])); ?>
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-gray-50 p-3 rounded-xl">
                      <p class="text-xs text-gray-500 mb-1"><i class="fas fa-calendar-times mr-1"></i>Old Contract End</p>
                      <p class="font-semibold text-gray-900"><?php echo date('M d, Y', strtotime($renewal['old_expiration'])); ?></p>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-xl">
                      <p class="text-xs text-gray-500 mb-1"><i class="fas fa-money-bill-wave mr-1"></i>Monthly Rate</p>
                      <p class="font-semibold text-gray-900">₱<?php echo number_format($renewal['monthly_rate'], 2); ?></p>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-xl">
                      <p class="text-xs text-gray-500 mb-1"><i class="fas fa-receipt mr-1"></i>Total Amount</p>
                      <p class="font-semibold text-emerald-600 text-lg">₱<?php echo number_format($renewal['total_amount'], 2); ?></p>
                    </div>
                  </div>
                  ...
                  <?php if ($renewal['late_renewal_fee'] > 0): ?>
                    <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded-xl">
                      <p class="text-sm text-red-700">
                        ...
                        <strong>Late Renewal Fee:</strong> ₱<?php echo number_format($renewal['late_renewal_fee'], 2); ?>
                      </p>
                    </div>
                  <?php endif; ?>
                  ...
                  <?php if ($renewal['payment_proof']): ?>
                    <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                      <div class="flex items-center justify-between">
                        ...
                        <a href="<?php echo htmlspecialchars($renewal['payment_proof']); ?>" target="_blank" class="text-blue-600 hover:text-blue-700 font-medium text-sm">
                  ...
                  <div class="flex gap-3 pt-4 border-t border-gray-200">
                    <a href="renewal_approval.php?id=<?php echo $renewal['id']; ?>&action=approve" class="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 font-semibold shadow-md">
                      <i class="fas fa-check-circle text-lg"></i>
                      <span>Approve Renewal</span>
                    </a>
                    <a href="renewal_approval.php?id=<?php echo $renewal['id']; ?>&action=decline" class="flex-1 bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 font-semibold shadow-md">
              ...
              <table class="w-full min-w-[1000px]">
                <thead class="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Request Type</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Stall</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                      <td class="px-6 py-4">
                        <div class="flex items-center">
                          ...
                          <div>
                            <div class="font-semibold text-gray-900"><?php echo htmlspecialchars($renewal['tradename'] ?? $renewal['username']); ?></div>
                            <div class="text-sm text-gray-500"><?php echo htmlspecialchars($renewal['user_email']); ?></div>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                      ...
                      <td class="px-6 py-4">
                        <span class="px-2 py-1 text-xs font-semibold rounded-full <?php echo $renewal['request_type'] === 'renewal' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'; ?>">
                          <?php echo ucfirst($renewal['request_type']); ?>
                        </span>
                      </td>
                      <td class="px-6 py-4 text-sm text-gray-900"><?php echo htmlspecialchars($renewal['stall_number'] ?? 'N/A'); ?></td>
                      <td class="px-6 py-4">
                        <div class="font-semibold text-gray-900">₱<?php echo number_format($renewal['total_amount'], 2); ?></div>
                        <?php if ($renewal['late_renewal_fee'] > 0): ?>
                          <div class="text-xs text-red-600">+ ₱<?php echo number_format($renewal['late_renewal_fee'], 2); ?> late fee</div>
                        <?php endif; ?>
                      </td>
                      <td class="px-6 py-4 text-sm text-gray-500"><?php echo date('M d, Y', strtotime($renewal['submitted_at'])); ?></td>
                      <td class="px-6 py-4">
                        <button onclick="viewDocuments(<?php echo htmlspecialchars(json_encode($renewal)); ?>)" class="text-blue-600 hover:text-blue-700 font-medium text-sm">
                          <i class="fas fa-folder-open mr-1"></i>View Documents
                        </button>
                      </td>
                      <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                          <button onclick="showApprovalModal(<?php echo $renewal['id']; ?>, '<?php echo addslashes(htmlspecialchars($renewal['tradename'] ?? $renewal['username'])); ?>', '<?php echo addslashes(htmlspecialchars($renewal['user_email'])); ?>')" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">
                            <i class="fas fa-check mr-1"></i>Approve
                          </button>
                          <button onclick="showDeclineModal(<?php echo $renewal['id']; ?>, '<?php echo addslashes(htmlspecialchars($renewal['tradename'] ?? $renewal['username'])); ?>', '<?php echo addslashes(htmlspecialchars($renewal['user_email'])); ?>')" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">
      ...
      <?php if ($expiringContracts): ?>
        <div>
          <h2 class="text-xl font-bold text-gray-900 mb-4">
            <i class="fas fa-exclamation-triangle text-orange-600 mr-2"></i>Expiring Contracts (Next 30 Days)
          </h2>
          <div class="grid grid-cols-1 gap-4">
            <?php foreach ($expiringContracts as $contract): ?>
              ...
              ?>
              <div class="gradient-card rounded-2xl shadow-lg overflow-hidden border-l-4 border-<?php echo $statusColor; ?>-500">
                <div class="p-6">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <h3 class="text-lg font-bold text-gray-900"><?php echo htmlspecialchars($contract['tradename']); ?></h3>
                          <p class="text-sm text-gray-500">
                            <i class="fas fa-store mr-1"></i>
                            Stall: <?php echo htmlspecialchars($contract['stall_number'] ?? 'N/A'); ?>
                          </p>
                        </div>
                      </div>
                    </div>
                    <span class="px-3 py-1 rounded-full text-sm font-semibold bg-<?php echo $statusColor; ?>-100 text-<?php echo $statusColor; ?>-700">
                      <?php echo $statusText; ?>
                    </span>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                    <div class="bg-gray-50 p-3 rounded-lg">
                      ...
                      <p class="font-semibold text-gray-900"><?php echo date('M d, Y', strtotime($contract['lease_start_date'])); ?></p>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-lg">
                      <p class="text-xs text-gray-500">Contract End</p>
                      <p class="font-semibold text-gray-900"><?php echo date('M d, Y', strtotime($contract['lease_expiration_date'])); ?></p>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-lg">
                      <p class="text-xs text-gray-500">Contact</p>
                      <p class="font-semibold text-gray-900 text-sm"><?php echo htmlspecialchars($contract['mobile'] ?? $contract['email']); ?></p>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <section id="paymentsSection" class="hidden space-y-6 animate-fade-in">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-gray-600 mt-1">Review and approve tenant payment submissions</p>
        </div>
              ...
              <tbody class="divide-y divide-gray-100">
                <?php foreach ($payments as $pay): ?>
                  ...
                  <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        <div class="w-12 h-12 bg-gradient-to-br from-blue-400 to-purple-500 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg">
                          <?php echo strtoupper(substr($pay['username'], 0, 2)); ?>
                        </div>
                        <div>
                          <p class="font-bold text-gray-900"><?php echo htmlspecialchars($pay['username']); ?></p>
                          <p class="text-xs text-gray-500 font-medium">ID: <?php echo (int)$pay['user_id']; ?></p>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="font-medium text-gray-700"><?php echo htmlspecialchars($pay['tenant_representative'] ?? 'N/A'); ?></span>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-center">
                      <div class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-xl">
                        <i class="fas fa-peso-sign text-emerald-600"></i>
                        <span class="font-bold text-emerald-700">₱<?php echo number_format($totalAmount, 2); ?></span>
                      </div>
                    </td>
                    <td class="px-6 py-4 text-center">
                      <div class="text-center">
                        <p class="font-semibold text-gray-800"><?php echo date('M d, Y', strtotime($pay['payment_date'])); ?></p>
                        <p class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($pay['payment_date'])); ?></p>
                      </div>
                    </td>
                    <td class="px-6 py-4 text-center">
                      <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-semibold bg-<?php echo $statusColor; ?>-100 text-<?php echo $statusColor; ?>-700">
                    ...
                    <td class="px-6 py-4">
                      <div class="flex items-center justify-center gap-2">
                        <!-- View Payment Button -->
                        <button onclick="window.open('<?php echo htmlspecialchars($pay['payment_image']); ?>', '_blank')" 
                        ...
                        <?php if (strtolower($pay['status']) !== 'approved' && strtolower($pay['status']) !== 'declined'): ?>
                          <!-- Approve Button -->
                          <button onclick="confirmPaymentAction(<?php echo $pay['id']; ?>, 'approved', '<?php echo addslashes(htmlspecialchars($pay['username'])); ?>')" 
                          ...
                          <!-- Decline Button -->
                          <button onclick="confirmPaymentAction(<?php echo $pay['id']; ?>, 'declined', '<?php echo addslashes(htmlspecialchars($pay['username'])); ?>')" 
                        ...
                        <?php else: ?>
                          <!-- Processed Indicator -->
                          <div class="p-2 bg-gray-100 text-gray-400 rounded-lg" title="Already processed">
    ...
    <div id="billingModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div class="bg-white rounded-[2rem] shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden animate-fade-in flex flex-col border border-gray-100">
        <!-- Modal Header -->
        <div class="bg-gradient-to-r from-emerald-600 via-blue-600 to-indigo-700 px-8 py-6 flex items-center justify-between shadow-lg relative overflow-hidden">
          ...
          <div class="relative z-10 flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-emerald-100/80 text-sm font-medium">Update utility bills for all active tenants</p>
            </div>
          </div>
          <div class="relative z-10 flex items-center gap-3">
            <div class="hidden md:flex bg-white/10 backdrop-blur-sm border border-white/20 px-4 py-2 rounded-xl text-white font-bold text-sm items-center gap-2">
              <i class="fas fa-users-cog"></i>
              <span><?php echo count($billingTenants); ?> Active Tenants</span>
            </div>
            <button type="button" onclick="closeBillingModal()" class="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all border border-white/20 hover:scale-110">
              <i class="fas fa-times"></i>
            </button>
          </div>
        </div>
        ...
        <form method="post" enctype="multipart/form-data" class="flex-1 flex flex-col overflow-hidden">
          ...
          <div class="px-8 py-6 bg-gray-50/50 border-b border-gray-100">
            <div class="flex flex-col md:flex-row gap-6 items-center justify-between">
              <div class="w-full md:w-auto">
                <label class="block text-xs font-black text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2 px-1">
              ...
              <div class="flex items-center gap-4 w-full md:w-auto">
                <button type="button" onclick="closeBillingModal()" class="flex-1 md:flex-none px-6 py-3.5 bg-white border-2 border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-50 font-bold transition-all flex items-center justify-center gap-2">
                  Cancel
                </button>
                <button type="button" onclick="confirmSaveBills()" class="flex-1 md:flex-auto px-8 py-3.5 bg-gradient-to-r from-emerald-500 to-blue-600 text-white rounded-2xl hover:from-emerald-600 hover:to-blue-700 font-bold shadow-xl shadow-blue-500/20 transition-all transform hover:scale-[1.02] flex items-center justify-center gap-3">
          ...
          <div class="flex-1 overflow-y-auto px-8 py-6 custom-scrollbar bg-white">
              ...
              <table class="w-full border-collapse">
                <thead class="bg-gray-50 sticky top-0 z-20">
                  <tr class="border-b-2 border-gray-100">
                    <th class="px-6 py-5 text-left text-xs font-black text-gray-400 uppercase tracking-widest bg-gray-50">Tenant Details</th>
                    <th class="px-6 py-5 text-left text-xs font-black text-gray-400 uppercase tracking-widest bg-gray-50">Unit</th>
                    <th class="px-4 py-5 text-center text-xs font-black text-gray-400 uppercase tracking-widest bg-gray-50">Utility Bill Photo</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr>
                      <td colspan="3" class="px-6 py-20 text-center">
                        <div class="flex flex-col items-center gap-4">
                          <div class="w-16 h-16 bg-gray-50 text-gray-300 rounded-2xl flex items-center justify-center">
                            <i class="fas fa-users-slash text-2xl"></i>
                          </div>
                          <p class="text-gray-400 font-bold">No active tenants available for billing</p>
                        </div>
                      </td>
                    </tr>
                  <?php else: ?>
                    ...
                    ?>
                      <tr class="group hover:bg-blue-50/30 transition-colors">
                        <td class="px-6 py-5">
                          <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-2xl flex items-center justify-center text-white font-black shadow-md transform group-hover:scale-110 transition-transform">
                              <?php echo strtoupper(substr($t['username'], 0, 2)); ?>
                            </div>
                            <div>
                              <p class="font-black text-gray-900 leading-tight"><?php echo htmlspecialchars($t['username']); ?></p>
                              <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mt-0.5">ID: <?php echo (int)$t['user_id']; ?></p>
                            </div>
                          </div>
                        </td>
                        <td class="px-6 py-5">
                          <div class="inline-flex flex-col">
                            <span class="px-3 py-1 bg-white border border-purple-100 text-purple-700 rounded-lg text-xs font-black shadow-sm mb-1">
                              <?php echo htmlspecialchars($t['stall_number']); ?>
                            </span>
                            <span class="text-[10px] text-gray-500 font-bold truncate max-w-[120px]"><?php echo htmlspecialchars($t['stall_location']); ?></span>
                          </div>
                        </td>
                        <td class="px-4 py-5">
                          <div class="flex flex-col items-center justify-center">
                            <!-- Hidden inputs to store modal values -->
                            <input type="hidden" name="water_bill[<?php echo $key; ?>]" id="water_amount_<?php echo $key; ?>" value="0">
                            <input type="hidden" name="electric_bill[<?php echo $key; ?>]" id="electric_amount_<?php echo $key; ?>" value="0">
                            
                            <!-- Hidden file inputs -->
                            <input type="file" name="bill_image[<?php echo $key; ?>]" id="water_file_<?php echo $key; ?>" class="hidden" onchange="updateIndividualBillStatus('<?php echo $key; ?>')">
                            <input type="file" name="electric_bill_image[<?php echo $key; ?>]" id="electric_file_<?php echo $key; ?>" class="hidden" onchange="updateIndividualBillStatus('<?php echo $key; ?>')">
                            
                            <button type="button" onclick="openIndividualBillModal('<?php echo $key; ?>', '<?php echo addslashes($t['username']); ?>', '<?php echo addslashes(htmlspecialchars($t['stall_number'])); ?>')" 
                              ...
                              <span class="text-[9px] font-bold text-gray-500 uppercase">Configure Bill</span>
                            </button>
                            
                            <!-- Status Badges -->
                            <div id="status_badges_<?php echo $key; ?>" class="mt-2 flex flex-wrap gap-1 justify-center hidden">
                              <span id="badge_water_<?php echo $key; ?>" class="hidden px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-[8px] font-black uppercase">W</span>
                              <span id="badge_electric_<?php echo $key; ?>" class="hidden px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-[8px] font-black uppercase">E</span>
                            </div>
                          </div>
                        </td>
                      </tr>
            ...
            <!-- Bottom Summary -->
            <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between text-[11px] font-black text-gray-400 uppercase tracking-widest">
          ...
          <div id="billingModalConfirmView" class="hidden flex-1 overflow-y-auto px-8 py-10 bg-white">
            <div class="max-w-2xl mx-auto space-y-8">
              <div class="text-center">
                <div class="w-20 h-20 bg-blue-50 text-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-sm border border-blue-100">
                  <i class="fas fa-clipboard-check text-3xl"></i>
                </div>
                <h3 class="text-2xl font-black text-gray-900 tracking-tight">Review Uploads</h3>
                <p class="text-gray-500 font-bold mt-1">Please confirm the details before finalizing</p>
              </div>

              <div class="bg-blue-50/50 border-2 border-dashed border-blue-200 rounded-2xl p-6 flex items-center justify-center gap-4">
              ...
              <div class="grid grid-cols-1 gap-6">
                <div class="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 text-center shadow-sm">
                  ...
                  <p class="text-[11px] text-emerald-700 font-black uppercase tracking-widest mb-1">Bill Photos</p>
                  <p class="text-4xl font-black text-emerald-900" id="confirmBillCount">0</p>
                </div>
              </div>
              
              <div class="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 flex items-center justify-between shadow-sm">
                <div class="flex items-center gap-4">
                  <div class="w-12 h-12 bg-emerald-500 rounded-2xl flex items-center justify-center text-white shadow-lg">
                    <i class="fas fa-users text-xl"></i>
                  </div>
                  <div>
                    <p class="text-[11px] font-black text-emerald-900 uppercase tracking-widest">Affected Tenants</p>
              ...
              <div class="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-4">
                ...
                <p class="text-amber-900 text-sm font-bold leading-relaxed">
                  Finalizing will automatically set amounts to PHP 0.00 and notify tenants to check their dashboards for verification.
                </p>
              </div>
              
              <div class="flex gap-4 pt-4 pb-10">
                <button type="button" onclick="backToBillingForm()" class="flex-1 px-8 py-5 bg-white border-2 border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-50 transition-all font-black uppercase tracking-widest text-xs">
                  <i class="fas fa-arrow-left mr-2"></i> Go Back
                </button>
                <button type="button" onclick="proceedSaveBills(this)" class="flex-1 px-8 py-5 bg-gradient-to-r from-emerald-500 to-blue-600 text-white rounded-2xl hover:scale-[1.02] hover:shadow-2xl transition-all font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-500/20">
      ...
      <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-4 border-b border-green-200">
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button onclick="sendExpirationNotifications()" class="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-all shadow-md">
                <i class="fas fa-bell mr-1"></i>Send Notification
              </button>
              <button onclick="selectAllContracts()" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg font-semibold text-sm transition-all">
            ...
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">
                  <input type="checkbox" id="selectAllCheckbox" onchange="toggleAllContracts()" class="rounded border-gray-300 text-orange-500 focus:ring-orange-500">
                </th>
                ...
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Stall #</th>
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Monthly Rate</th>
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Contract Start</th>
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Contract End</th>
                <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Status</th>
                <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Days Remaining</th>
                <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <?php else: ?>
                ...
                ?>
                  <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 text-center">
                      <input type="checkbox" class="contract-checkbox rounded border-gray-300 text-orange-500 focus:ring-orange-500" 
                    ...
                    <td class="px-6 py-4">
                      <div class="font-semibold text-gray-900"><?php echo htmlspecialchars($contract['company_name']); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-gray-600"><?php echo htmlspecialchars($contract['email']); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-gray-600"><?php echo htmlspecialchars($contract['mobile']); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-gray-900 font-medium"><?php echo htmlspecialchars($contract['stall_number'] ?? 'N/A'); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-gray-900 font-medium">₱<?php echo number_format($contract['monthly_rate'] ?? 0, 2); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-gray-600"><?php echo date('M d, Y', strtotime($contract['lease_start_date'])); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-gray-900 font-medium"><?php echo date('M d, Y', strtotime($contract['lease_end_date'])); ?></div>
                    </td>
                    <td class="px-6 py-4 text-center">
                    ...
                    <td class="px-6 py-4 text-center">
                      <div class="font-medium <?php echo $isExpiringSoon ? 'text-red-600' : 'text-gray-600'; ?>">
                        <?php echo $daysRemaining; ?> days
                      </div>
                    </td>
                    <td class="px-6 py-4 text-center border-l border-gray-100">
                      <button onclick="openNotifyBalanceModal(<?php echo $contract['tenant_detail_id']; ?>, '<?php echo addslashes(htmlspecialchars($contract['company_name'])); ?>', '<?php echo addslashes(htmlspecialchars($contract['email'])); ?>', '<?php echo addslashes(htmlspecialchars($contract['mobile'])); ?>', <?php echo $contract['monthly_rate'] ?? 0; ?>, '<?php echo addslashes($contract['lease_end_date']); ?>')" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-1 mx-auto">
    ...
    <section id="historySection" class="hidden space-y-6 animate-fade-in">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <p class="text-gray-600 mt-1">View comprehensive reports for applications, payments, billing, work permits, and renewals</p>
        </div>
      </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 border-b border-blue-200">
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <button onclick="printApplications()" class="bg-blue-100 hover:bg-blue-200 text-blue-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-print mr-1"></i>Print
                </button>
                <button onclick="exportApplications()" class="bg-green-100 hover:bg-green-200 text-green-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Applicant Details</th>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Business Information</th>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Stall Details</th>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Contact Information</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Application Status</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Date Applied</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <?php else: ?>
                  <?php foreach ($unifiedApplications as $app): ?>
                    <tr class="hover:bg-gray-50 transition-colors">
                      <td class="px-6 py-4">
                        <div class="flex items-center">
                          <div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 text-white flex items-center justify-center font-bold text-sm shadow-md">
                            <?php echo strtoupper(substr($app['tradename'] ?? 'A', 0, 1)); ?>
                          </div>
                          <div class="ml-4">
                            <div class="text-sm font-bold text-gray-900"><?php echo htmlspecialchars($app['tradename'] ?? 'N/A'); ?></div>
                            <div class="text-sm text-gray-500"><?php echo htmlspecialchars($app['username'] ?? 'N/A'); ?></div>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="space-y-1">
                          <div class="text-sm font-semibold text-gray-900"><?php echo htmlspecialchars($app['company_name'] ?? 'N/A'); ?></div>
                      ...
                      <td class="px-6 py-4">
                        <div class="space-y-1">
                          <div class="flex items-center">
                            ...
                            <span class="text-sm font-semibold text-gray-900"><?php echo htmlspecialchars($app['stall_number'] ?? 'N/A'); ?></span>
                          </div>
                          <div class="flex items-center">
                            <i class="fas fa-peso-sign text-gray-400 mr-2 text-xs"></i>
                            <span class="text-sm text-gray-600">₱<?php echo number_format($app['monthly_rate'] ?? 0, 2); ?>/mo</span>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="space-y-1">
                          <div class="flex items-center">
                            ...
                            <span class="text-sm text-gray-900"><?php echo htmlspecialchars($app['email'] ?? 'N/A'); ?></span>
                          </div>
                          <div class="flex items-center">
                            <i class="fas fa-phone text-gray-400 mr-2 text-xs"></i>
                            <span class="text-sm text-gray-600"><?php echo htmlspecialchars($app['mobile'] ?? 'N/A'); ?></span>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-<?php echo $app['status_color'] === 'success' ? 'green' : ($app['status_color'] === 'error' ? 'red' : 'yellow'); ?>-100 text-<?php echo $app['status_color'] === 'success' ? 'green' : ($app['status_color'] === 'error' ? 'red' : 'yellow'); ?>-800">
                          <i class="fas fa-<?php echo $app['status_color'] === 'success' ? 'check' : ($app['status_color'] === 'error' ? 'times' : 'clock'); ?>-circle mr-1"></i>
                          <?php echo $app['application_status']; ?>
                        </span>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="text-sm text-gray-900 font-medium"><?php echo date('M d, Y', strtotime($app['created_at'])); ?></div>
                        <div class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($app['created_at'])); ?></div>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div class="bg-gradient-to-r from-emerald-50 to-teal-50 p-4 border-b border-emerald-200">
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <button onclick="printPayments()" class="bg-emerald-100 hover:bg-emerald-200 text-emerald-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-print mr-1"></i>Print
                </button>
                <button onclick="exportPayments()" class="bg-green-100 hover:bg-green-200 text-green-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-download mr-1"></i>Export
                </button>
              </div>
            </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tenant Information</th>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Stall Details</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Payment Amount</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Payment Type</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Payment Method</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Payment Status</th>
                ...
                <?php else: ?>
                  <?php foreach ($unifiedPayments as $payment): ?>
                    <tr class="hover:bg-gray-50 transition-colors">
                      <td class="px-6 py-4">
                        <div class="flex items-center">
                          <div class="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-500 to-teal-500 text-white flex items-center justify-center font-bold text-sm shadow-md">
                            <?php echo strtoupper(substr($payment['tradename'] ?? 'T', 0, 1)); ?>
                          </div>
                          <div class="ml-4">
                            <div class="text-sm font-bold text-gray-900"><?php echo htmlspecialchars($payment['tradename'] ?? $payment['username'] ?? 'N/A'); ?></div>
                            <div class="text-sm text-gray-500"><?php echo htmlspecialchars($payment['email'] ?? 'N/A'); ?></div>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="space-y-1">
                          <div class="flex items-center">
                            <i class="fas fa-store text-gray-400 mr-2 text-xs"></i>
                            <span class="text-sm font-semibold text-gray-900"><?php echo htmlspecialchars($payment['stall_number'] ?? 'N/A'); ?></span>
                          </div>
                          <div class="flex items-center">
                            <i class="fas fa-peso-sign text-gray-400 mr-2 text-xs"></i>
                            <span class="text-sm text-gray-600">₱<?php echo number_format($payment['monthly_rate'] ?? 0, 2); ?>/mo</span>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="text-lg font-bold text-emerald-600">₱<?php echo number_format($payment['amount'] ?? 0, 2); ?></div>
                        <?php if ($payment['billing_month']): ?>
                          <div class="text-xs text-gray-500"><?php echo date('M Y', strtotime($payment['billing_month'])); ?></div>
                        <?php endif; ?>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-800">
                          <i class="fas fa-tag mr-1"></i>
                          <?php echo ucfirst($payment['payment_type'] ?? 'N/A'); ?>
                        </div>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-800">
                      ...
                      <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-<?php echo $payment['payment_color'] === 'success' ? 'green' : ($payment['payment_color'] === 'error' ? 'red' : ($payment['payment_color'] === 'warning' ? 'yellow' : 'gray')); ?>-100 text-<?php echo $payment['payment_color'] === 'success' ? 'green' : ($payment['payment_color'] === 'error' ? 'red' : ($payment['payment_color'] === 'warning' ? 'yellow' : 'gray')); ?>-800">
                          <i class="fas fa-<?php echo $payment['payment_color'] === 'success' ? 'check' : ($payment['payment_color'] === 'error' ? 'times' : ($payment['payment_color'] === 'warning' ? 'clock' : 'minus')); ?>-circle mr-1"></i>
                          <?php echo $payment['payment_status_text']; ?>
                        </span>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <?php if ($payment['payment_date']): ?>
                          <div class="text-sm text-gray-900 font-medium"><?php echo date('M d, Y', strtotime($payment['payment_date'])); ?></div>
                          <div class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($payment['payment_date'])); ?></div>
                        <?php else: ?>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div class="bg-gradient-to-r from-orange-50 to-amber-50 p-4 border-b border-orange-200">
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <button onclick="printWorkPermits()" class="bg-orange-100 hover:bg-orange-200 text-orange-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-print mr-1"></i>Print
                </button>
                <button onclick="exportWorkPermits()" class="bg-green-100 hover:bg-green-200 text-green-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-download mr-1"></i>Export
                </button>
              </div>
            </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Permit Information</th>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tenant Details</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Total Charges</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Permit Status</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Date Filed</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  <?php foreach ($workPermitReports as $permit): ?>
                    <tr class="hover:bg-gray-50 transition-colors">
                      <td class="px-6 py-4">
                        <div class="space-y-2">
                          <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-amber-500 text-white flex items-center justify-center font-bold text-sm shadow-md">
                              <i class="fas fa-hard-hat"></i>
                            </div>
                            <div class="ml-3">
                              <div class="text-sm font-bold text-gray-900"><?php echo htmlspecialchars($permit['permit_no'] ?? 'N/A'); ?></div>
                              <div class="text-xs text-gray-500">Permit Number</div>
                            </div>
                          </div>
                          <div class="ml-3">
                            <div class="text-sm text-gray-900 max-w-xs truncate" title="<?php echo htmlspecialchars($permit['scope_of_work'] ?? ''); ?>">
                      ...
                      <td class="px-6 py-4">
                        <div class="space-y-1">
                          <div class="flex items-center">
                            ...
                            <span class="text-sm font-semibold text-gray-900"><?php echo htmlspecialchars($permit['tenant_name'] ?? 'N/A'); ?></span>
                          </div>
                          <div class="flex items-center">
                            <i class="fas fa-store text-gray-400 mr-2 text-xs"></i>
                            <span class="text-sm text-gray-600"><?php echo htmlspecialchars($permit['stall_number'] ?? 'N/A'); ?></span>
                          </div>
                          <div class="flex items-center">
                            <i class="fas fa-envelope text-gray-400 mr-2 text-xs"></i>
                            <span class="text-sm text-gray-600"><?php echo htmlspecialchars($permit['email'] ?? 'N/A'); ?></span>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="text-lg font-bold text-orange-600">₱<?php echo number_format($permit['total_charges'] ?? 0, 2); ?></div>
                      ...
                      <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-<?php echo $permit['permit_color'] === 'success' ? 'green' : ($permit['permit_color'] === 'error' ? 'red' : ($permit['permit_color'] === 'warning' ? 'yellow' : 'gray')); ?>-100 text-<?php echo $permit['permit_color'] === 'success' ? 'green' : ($permit['permit_color'] === 'error' ? 'red' : ($permit['permit_color'] === 'warning' ? 'yellow' : 'gray')); ?>-800">
                          <i class="fas fa-<?php echo $permit['permit_color'] === 'success' ? 'check' : ($permit['permit_color'] === 'error' ? 'times' : 'clock'); ?>-circle mr-1"></i>
                          <?php echo $permit['permit_status_text']; ?>
                        </span>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="text-sm text-gray-900 font-medium"><?php echo date('M d, Y', strtotime($permit['date_filed'])); ?></div>
                        <div class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($permit['date_filed'])); ?></div>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div class="bg-gradient-to-r from-teal-50 to-cyan-50 p-4 border-b border-teal-200">
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <button onclick="printRenewals()" class="bg-teal-100 hover:bg-teal-200 text-teal-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-print mr-1"></i>Print
                </button>
                <button onclick="exportRenewals()" class="bg-green-100 hover:bg-green-200 text-green-700 px-3 py-1 rounded-lg font-semibold text-sm transition-all">
                  <i class="fas fa-download mr-1"></i>Export
                </button>
              </div>
            </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tenant Information</th>
                  <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Renewal Details</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Renewal Amount</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Renewal Status</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Payment Status</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Submitted Date</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <?php else: ?>
                  <?php foreach ($renewalReports as $renewal): ?>
                    <tr class="hover:bg-gray-50 transition-colors">
                      <td class="px-6 py-4">
                        <div class="flex items-center">
                          <div class="w-12 h-12 rounded-full bg-gradient-to-br from-teal-500 to-cyan-500 text-white flex items-center justify-center font-bold text-sm shadow-md">
                            <?php echo strtoupper(substr($renewal['tradename'] ?? 'R', 0, 1)); ?>
                          </div>
                          <div class="ml-4">
                            <div class="text-sm font-bold text-gray-900"><?php echo htmlspecialchars($renewal['tradename'] ?? $renewal['username'] ?? 'N/A'); ?></div>
                            <div class="text-sm text-gray-500"><?php echo htmlspecialchars($renewal['stall_number'] ?? 'N/A'); ?></div>
                          </div>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="space-y-1">
                          ...
                          <div class="text-sm text-gray-600"><?php echo ucfirst(str_replace('_', ' ', $renewal['business_type'] ?? 'N/A')); ?></div>
                        </div>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="text-lg font-bold text-teal-600">₱<?php echo number_format($renewal['total_amount'] ?? 0, 2); ?></div>
                      ...
                      <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-<?php echo $renewal['renewal_color'] === 'success' ? 'green' : ($renewal['renewal_color'] === 'error' ? 'red' : ($renewal['renewal_color'] === 'warning' ? 'yellow' : 'gray')); ?>-100 text-<?php echo $renewal['renewal_color'] === 'success' ? 'green' : ($renewal['renewal_color'] === 'error' ? 'red' : ($renewal['renewal_color'] === 'warning' ? 'yellow' : 'gray')); ?>-800">
                          <i class="fas fa-<?php echo $renewal['renewal_color'] === 'success' ? 'check' : ($renewal['renewal_color'] === 'error' ? 'times' : 'clock'); ?>-circle mr-1"></i>
                      ...
                      <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-<?php echo $renewal['payment_status_text'] === 'Paid' ? 'green' : ($renewal['payment_status_text'] === 'Declined' ? 'red' : ($renewal['payment_status_text'] === 'Pending' ? 'yellow' : 'gray')); ?>-100 text-<?php echo $renewal['payment_status_text'] === 'Paid' ? 'green' : ($renewal['payment_status_text'] === 'Declined' ? 'red' : ($renewal['payment_status_text'] === 'Pending' ? 'yellow' : 'gray')); ?>-800">
                          <i class="fas fa-<?php echo $renewal['payment_status_text'] === 'Paid' ? 'check' : ($renewal['payment_status_text'] === 'Declined' ? 'times' : ($renewal['payment_status_text'] === 'Pending' ? 'clock' : 'minus')); ?>-circle mr-1"></i>
                          <?php echo $renewal['payment_status_text']; ?>
                        </span>
                      </td>
                      <td class="px-6 py-4 text-center">
                        <div class="text-sm text-gray-900 font-medium"><?php echo date('M d, Y', strtotime($renewal['submitted_at'])); ?></div>
                        <div class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($renewal['submitted_at'])); ?></div>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
          ...
          <?php else: ?>
            <div class="space-y-4">
              <?php foreach ($declinedApplications as $app): ?>
                <div class="bg-gradient-to-r from-red-50 to-pink-50 rounded-xl p-5 border-l-4 border-red-500 shadow-sm hover:shadow-md transition">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center gap-3 mb-2">
                        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-red-600 to-pink-500 text-white flex items-center justify-center font-bold text-lg shadow-lg">
                          <?php echo strtoupper(substr($app['tradename'] ?? 'T', 0, 1)); ?>
                        </div>
                        <div>
                          <h4 class="font-bold text-lg text-gray-900"><?php echo htmlspecialchars($app['tradename'] ?? 'N/A'); ?></h4>
                          <p class="text-sm text-gray-600"><?php echo htmlspecialchars($app['company_name'] ?? 'N/A'); ?></p>
                        </div>
                      </div>
                      <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-3">
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-briefcase mr-1"></i>Business Type</p>
                          <p class="font-semibold text-gray-800"><?php echo htmlspecialchars(ucfirst($app['business_type'] ?? 'N/A')); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-envelope mr-1"></i>Email</p>
                          <p class="font-semibold text-gray-800 text-sm"><?php echo htmlspecialchars($app['email'] ?? 'N/A'); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-calendar mr-1"></i>Declined Date</p>
                          <p class="font-semibold text-gray-800"><?php echo date('M d, Y', strtotime($app['declined_date'])); ?></p>
                        </div>
                      </div>
                    </div>
                    <div class="ml-4">
                      <span class="inline-flex items-center gap-2 px-4 py-2 bg-red-100 text-red-700 rounded-full font-semibold text-sm">
          ...
          <?php else: ?>
            <div class="space-y-4">
              <?php foreach ($paidHistory as $payment): ?>
                <div class="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-5 border-l-4 border-emerald-500 shadow-sm hover:shadow-md transition">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center gap-3 mb-2">
                        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-600 to-teal-500 text-white flex items-center justify-center font-bold text-lg shadow-lg">
                          <?php echo strtoupper(substr($payment['tradename'] ?? 'T', 0, 1)); ?>
                        </div>
                        <div>
                          <h4 class="font-bold text-lg text-gray-900"><?php echo htmlspecialchars($payment['tradename'] ?? $payment['username'] ?? 'N/A'); ?></h4>
                          <p class="text-sm text-gray-600"><?php echo htmlspecialchars($payment['company_name'] ?? 'N/A'); ?></p>
                        </div>
                      </div>
                      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-store mr-1"></i>Stall</p>
                          <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($payment['stall_number'] ?? 'N/A'); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-envelope mr-1"></i>Email</p>
                          <p class="font-semibold text-gray-800 text-sm"><?php echo htmlspecialchars($payment['email'] ?? 'N/A'); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-phone mr-1"></i>Mobile</p>
                          <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($payment['mobile'] ?? 'N/A'); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-calendar mr-1"></i>Payment Date</p>
                          <p class="font-semibold text-gray-800"><?php echo date('M d, Y', strtotime($payment['payment_date'])); ?></p>
                        </div>
                      </div>
                    </div>
                    <div class="ml-4">
                      <span class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-100 text-emerald-700 rounded-full font-semibold text-sm">
          ...
          <?php else: ?>
            <div class="space-y-4">
              <?php foreach ($unpaidHistory as $tenant): ?>
                <div class="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl p-5 border-l-4 border-orange-500 shadow-sm hover:shadow-md transition">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center gap-3 mb-2">
                        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-orange-600 to-yellow-500 text-white flex items-center justify-center font-bold text-lg shadow-lg">
                          <?php echo strtoupper(substr($tenant['tradename'] ?? 'T', 0, 1)); ?>
                        </div>
                        <div>
                          <h4 class="font-bold text-lg text-gray-900"><?php echo htmlspecialchars($tenant['tradename'] ?? 'N/A'); ?></h4>
                          <p class="text-sm text-gray-600"><?php echo htmlspecialchars($tenant['company_name'] ?? 'N/A'); ?></p>
                        </div>
                      </div>
                      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-store mr-1"></i>Stall</p>
                          <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($tenant['stall_number'] ?? 'N/A'); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-peso-sign mr-1"></i>Monthly Rate</p>
                          <p class="font-semibold text-gray-800">₱<?php echo number_format($tenant['monthly_rate'] ?? 0, 2); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-envelope mr-1"></i>Email</p>
                          <p class="font-semibold text-gray-800 text-sm"><?php echo htmlspecialchars($tenant['email'] ?? 'N/A'); ?></p>
                        </div>
                        <div>
                          <p class="text-xs text-gray-500 mb-1"><i class="fas fa-phone mr-1"></i>Mobile</p>
                          <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($tenant['mobile'] ?? 'N/A'); ?></p>
                        </div>
                      </div>
                    </div>
                    <div class="flex flex-col items-end gap-3 ml-4">
                      <span class="inline-flex items-center gap-2 px-4 py-2 bg-orange-100 text-orange-700 rounded-full font-semibold text-sm">
                        <i class="fas fa-exclamation-triangle"></i> Unpaid
                      </span>
                      <button onclick="sendDueDateReminder(this, <?php echo $tenant['user_id']; ?>, <?php echo $tenant['monthly_rate'] ?? 0; ?>)" class="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm hover:shadow-md flex items-center gap-2">
    ...
    <!-- SOA List Modal -->
    <div id="soaListModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden animate-fade-in flex flex-col border border-gray-100">
        <!-- Modal Header -->
        <div class="bg-gradient-to-r from-blue-700 to-indigo-800 px-8 py-6 flex items-center justify-between shadow-lg relative overflow-hidden flex-shrink-0">
          <div class="absolute inset-0 bg-white/10 opacity-20 pointer-events-none"></div>
          <div class="relative z-10 flex items-center gap-4">
            <div class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/30 shadow-inner">
              <i class="fas fa-file-invoice text-white text-xl"></i>
            </div>
            <div>
              <h1 class="text-white font-black text-xl tracking-tight">Statement of Account</h1>
              <p class="text-blue-100/80 text-sm font-medium">Generate and manage tenant Statement of Accounts (SOA)</p>
            </div>
          </div>
          <div class="flex items-center gap-4 relative z-10">
            <div class="bg-white/20 backdrop-blur-md text-white px-4 py-2 rounded-xl font-semibold border border-white/30">
              <i class="fas fa-users mr-2"></i>
              <?php echo count($allTenants); ?> Active Tenants
            </div>
            <button type="button" onclick="closeSOAListModal()" class="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all border border-white/20 hover:scale-110">
              <i class="fas fa-times"></i>
            </button>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto w-full">
          <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200 sticky top-0 z-10 shadow-sm">
              <tr>
                <th class="px-6 py-4 text-left font-bold text-gray-800 uppercase text-xs tracking-wider">Tenant</th>
                <th class="px-6 py-4 text-left font-bold text-gray-800 uppercase text-xs tracking-wider">Business Name</th>
                <th class="px-6 py-4 text-center font-bold text-gray-800 uppercase text-xs tracking-wider">Stall</th>
                <th class="px-6 py-4 text-center font-bold text-gray-800 uppercase text-xs tracking-wider">Rate</th>
                <th class="px-6 py-4 text-center font-bold text-gray-800 uppercase text-xs tracking-wider">Actions</th>
              </tr>
            </thead>
                  ...
                  <tr class="hover:bg-blue-50/50 transition-colors">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          <p class="font-bold text-gray-900"><?php echo htmlspecialchars($t['tradename']); ?></p>
                          <p class="text-xs text-gray-500"><?php echo htmlspecialchars($t['email']); ?></p>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <p class="text-gray-700 font-medium"><?php echo htmlspecialchars($t['company_name']); ?></p>
                    </td>
                    <td class="px-6 py-4 text-center">
                      <span class="inline-flex items-center px-3 py-1 bg-purple-50 text-purple-700 border border-purple-200 rounded-lg text-xs font-bold">
                        <i class="fas fa-store mr-1.5 opacity-70"></i>
                        <?php echo htmlspecialchars($t['stall_number'] ?? 'N/A'); ?>
                      </span>
                    </td>
                    <td class="px-6 py-4 text-center">
                      <span class="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-sm font-black">
            ...
            <div class="gradient-card rounded-2xl shadow-lg overflow-hidden hover-lift border border-gray-100">
              <div class="p-6">
                <div class="flex items-start justify-between mb-4">
                  <div class="flex-1">
                    <div class="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <h3 class="text-xl font-bold text-gray-900"><?php echo htmlspecialchars($bir['tradename']); ?></h3>
                        <p class="text-sm text-gray-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div class="bg-gray-50 p-3 rounded-xl">
                    <p class="text-xs text-gray-500 mb-1"><i class="fas fa-envelope mr-1"></i>Email</p>
                    <p class="font-semibold text-gray-900 text-sm truncate"><?php echo htmlspecialchars($bir['email']); ?></p>
                  </div>
                  <div class="bg-gray-50 p-3 rounded-xl">
                    <p class="text-xs text-gray-500 mb-1"><i class="fas fa-phone mr-1"></i>Mobile</p>
                  ...
                  <div class="bg-gray-50 p-3 rounded-xl">
                    <p class="text-xs text-gray-500 mb-1"><i class="fas fa-money-bill-wave mr-1"></i>Renewal Amount</p>
                ...
                <?php if (!empty($bir['payment_proof'])): ?>
                  <div class="mb-4 p-4 bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-200 rounded-xl">
                    <div class="flex items-center justify-between">
                      ...
                      <a href="<?php echo htmlspecialchars($bir['payment_proof']); ?>" target="_blank" class="bg-white hover:bg-gray-50 text-blue-600 px-4 py-2 rounded-xl transition-all flex items-center gap-2 font-medium shadow-sm border border-blue-200">
                ...
                <!-- BIR Document -->
                <div class="mb-4 p-4 bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-xl">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                    ...
                    <a href="<?php echo htmlspecialchars($bir['bir_document']); ?>" target="_blank" class="bg-white hover:bg-gray-50 text-purple-600 px-4 py-2 rounded-xl transition-all flex items-center gap-2 font-medium shadow-sm border border-purple-200">
                      <i class="fas fa-external-link-alt"></i>
                ...
                <?php if ($bir['status'] === 'pending'): ?>
                  <div class="flex gap-3 pt-4 border-t border-gray-200">
                    <button onclick='openBIRApprovalModal(<?php echo json_encode(["id" => $bir["id"], "tradename" => addslashes($bir["tradename"])]); ?>)' class="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 font-semibold shadow-md">
                      <i class="fas fa-check-circle text-lg"></i>
                      <span>Approve</span>
                    </button>
                    <a href="bir_approval.php?id=<?php echo $bir['id']; ?>&action=decline" class="flex-1 bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 font-semibold shadow-md">
                      <i class="fas fa-times-circle text-lg"></i>
                      <span>Decline</span>
                    </a>
                  </div>
  ...
  <div id="applicantHistoryModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onclick="if(event.target===this)closeApplicantHistoryModal()">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col animate-fade-in">
      ...
      <div class="flex items-center justify-between px-8 py-5 border-b border-gray-100 flex-shrink-0">
        ...
        <div class="flex items-center gap-4">
          <div class="flex flex-col">
              ...
              <select id="applicantHistoryYearSelector" class="bg-gray-50 text-gray-900 rounded-xl px-4 py-2 border-0 focus:ring-2 focus:ring-blue-300 outline-none font-semibold text-xs shadow-inner min-w-[120px]">
          ...
          <span class="bg-blue-50 border border-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold" id="applicantHistoryCountBadge">
            <i class="fas fa-users mr-1"></i>
            <?php echo count($applicantHistory); ?> Applicant<?php echo count($applicantHistory) !== 1 ? 's' : ''; ?>
          </span>
          <button onclick="closeApplicantHistoryModal()" class="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
      ...
      <div class="overflow-y-auto flex-1 px-8 py-6 space-y-5">
        ...
        <?php if ($applicantHistory):
            $totalAttempts = 0; $totalApproved = 0; $totalDeclined = 0; $totalPending = 0;
        ...
        <div class="grid grid-cols-4 gap-3">
          <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-3 text-white flex items-center gap-3">
            <i class="fas fa-paper-plane text-xl text-blue-200"></i>
            <div><p class="text-xs text-blue-100 font-semibold uppercase tracking-wide">Total Attempts</p><p class="text-2xl font-bold" id="applicantStatTotalAttempts"><?php echo $totalAttempts; ?></p></div>
          </div>
          <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-3 text-white flex items-center gap-3">
            <i class="fas fa-check-circle text-xl text-emerald-200"></i>
            <div><p class="text-xs text-emerald-100 font-semibold uppercase tracking-wide">Approved</p><p class="text-2xl font-bold" id="applicantStatApproved"><?php echo $totalApproved; ?></p></div>
          </div>
          <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-2xl p-3 text-white flex items-center gap-3">
            <i class="fas fa-times-circle text-xl text-red-200"></i>
            <div><p class="text-xs text-red-100 font-semibold uppercase tracking-wide">Declined</p><p class="text-2xl font-bold" id="applicantStatDeclined"><?php echo $totalDeclined; ?></p></div>
          </div>
          <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-2xl p-3 text-white flex items-center gap-3">
            <i class="fas fa-clock text-xl text-amber-200"></i>
            <div><p class="text-xs text-amber-100 font-semibold uppercase tracking-wide">Pending</p><p class="text-2xl font-bold" id="applicantStatPending"><?php echo $totalPending; ?></p></div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
          <table class="w-full table-modern">
            <thead>
              <tr>
                <th class="px-6 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Applicant</th>
                <th class="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Business Type</th>
                <th class="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Attempts</th>
                <th class="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Status</th>
                <th class="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Date Applied</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100" id="applicantHistoryTableBody">
              <?php foreach ($applicantHistory as $applicant):
                ...
                if ($approved > 0)     { $badgeBg = '#d1fae5'; $badgeText = '#065f46'; $statusLabel = 'Approved';  $statusIcon = 'fa-check-circle'; }
                elseif ($declined > 0) { $badgeBg = '#fee2e2'; $badgeText = '#991b1b'; $statusLabel = 'Declined';  $statusIcon = 'fa-times-circle'; }
                else                   { $badgeBg = '#fef3c7'; $badgeText = '#92400e'; $statusLabel = 'Pending';   $statusIcon = 'fa-clock'; }
              ?>
              <?php
              ...
              <tr class="hover:bg-indigo-50 transition-colors cursor-pointer"
                ...
                <td class="px-6 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="font-semibold text-gray-900 text-sm"><?php echo htmlspecialchars($applicant['tradename'] ?? $applicant['username'] ?? 'Unknown'); ?></p>
                      <p class="text-xs text-gray-400"><?php echo htmlspecialchars($applicant['user_email'] ?? ''); ?></p>
                    </div>
                  </div>
                </td>
                <td class="px-4 py-3 text-center">
                  <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full"><?php echo htmlspecialchars(ucfirst($applicant['business_type'] ?? '—')); ?></span>
                </td>
                <td class="px-4 py-3 text-center">
                  <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-700 font-bold text-sm"><?php echo $attempts; ?></span>
                </td>
                <td class="px-4 py-3 text-center">
                  <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" style="background:<?php echo $badgeBg; ?>;color:<?php echo $badgeText; ?>">
  ...
  <!-- Edit Contract Modal -->
  <div id="editContractModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-8 animate-fade-in">
      ...
      <form id="editContractForm" method="post" action="update_contract.php" class="space-y-5">
        ...
        <div>
          <label for="lease_start_date" class="block text-sm font-bold text-gray-800 mb-2 flex items-center gap-2">
        ...
        <div>
          <label for="lease_expiration_date" class="block text-sm font-bold text-gray-800 mb-2 flex items-center gap-2">
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="text-sm text-blue-700">
              ...
              <div class="flex gap-2 mt-2">
                <button type="button" onclick="extendContract(6)" class="px-3 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-xs font-medium">
                  +6 Months
                </button>
                <button type="button" onclick="extendContract(12)" class="px-3 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-xs font-medium">
                  +1 Year
                </button>
                <button type="button" onclick="extendContract(24)" class="px-3 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-xs font-medium">
                  +2 Years
                </button>
              </div>
            </div>
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', function() {
      <?php if (isset($_SESSION['payment_success'])): ?>
        showToast('success', '<?php echo addslashes($_SESSION['payment_success']); unset($_SESSION['payment_success']); ?>');
      <?php endif; ?>
      
      <?php if (isset($_SESSION['payment_error'])): ?>
        showToast('error', '<?php echo addslashes($_SESSION['payment_error']); unset($_SESSION['payment_error']); ?>');
      <?php endif; ?>
    });
  ...
  <div id="paymentConfirmModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
      <div class="p-8">
        ...
        <div class="flex gap-3">
          <button onclick="closePaymentConfirmModal()" class="flex-1 px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-xl font-semibold transition-all">
  ...
  <!-- Per-Applicant History Modal -->
  <div id="applicantHistoryDetailModal" class="hidden fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4" onclick="if(event.target===this)closeHistoryModal()">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col animate-fade-in">

      <!-- Header -->
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-500 flex items-center justify-center" id="histModalAvatar"></div>
          <div>
            <h2 class="text-base font-bold text-gray-900" id="histModalName">—</h2>
            <p class="text-xs text-gray-400" id="histModalEmail">—</p>
          </div>
        </div>
        <button onclick="closeHistoryModal()" class="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
          <i class="fas fa-times text-gray-500"></i>
        </button>
      </div>
      ...
      <div class="overflow-y-auto flex-1 px-6 py-6 space-y-5">
        ...
        <div class="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl p-5 text-white text-center">
          <p class="text-sm text-indigo-100 font-semibold uppercase tracking-wider mb-1">Total Submission Attempts</p>
          ...
          <div class="mt-3 bg-white/20 rounded-full h-2 w-full">
            <div class="bg-white rounded-full h-2 transition-all" id="histModalProgressBar" style="width: 20%"></div>
          </div>
...
<button id="confirmYes" class="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-lg transition-all font-medium">
  Yes, Proceed
</button>
          <button onclick="closeConfirmModal()" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition-all font-medium">
            Cancel
          </button>
        </div>
      </div>
  ...
  <!-- Reports Modal -->
  <div id="reportsModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in">
          <!-- Header -->
          <div class="bg-gradient-to-r from-blue-600 to-emerald-600 px-8 py-6 flex items-center justify-between">
              ...
              <div class="flex items-center gap-4">
                  <div class="flex flex-col">
                      ...
                      <input type="month" id="reportMonthSelector" class="bg-white text-gray-900 rounded-xl px-4 py-2 border-0 focus:ring-2 focus:ring-blue-300 outline-none font-semibold shadow-inner" value="<?php echo date('Y-m'); ?>">
                  </div>
                  <button onclick="closeReportsModal()" class="w-10 h-10 rounded-full bg-white/20 hover:bg-white/30 text-white flex items-center justify-center transition-all">
                      <i class="fas fa-times"></i>
                  </button>
              </div>
          </div>
          ...
          <div class="bg-gray-50 border-b border-gray-200 px-8 py-4 flex items-center justify-between">
              <div class="flex items-center gap-6">
                  <div id="reportSummaryTotalSales" class="bg-white px-5 py-2.5 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <nav class="flex gap-2">
                      <button onclick="switchReportTab('sales')" class="report-tab-btn active px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="sales">Sales History</button>
                      <button onclick="switchReportTab('tenants')" class="report-tab-btn px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="tenants">Tenants List</button>
                      <button onclick="switchReportTab('pending')" class="report-tab-btn px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="pending">Pending Applications</button>
                      <button onclick="switchReportTab('outstanding')" class="report-tab-btn px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="outstanding">Outstanding Balances</button>
                  </nav>
              </div>
              <button onclick="printReport()" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg hover:shadow-xl">
                  ...
                  <div class="bg-white rounded-3xl border border-gray-200 overflow-hidden shadow-sm">
                      <table class="w-full text-left">
                          <thead class="bg-gray-50 border-b border-gray-100">
                              <tr>
                                  <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase">Transaction Date</th>
                                  ...
                                  <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase text-right">Amount Paid</th>
                              </tr>
                          </thead>
              ...
              <div id="reportTab-tenants" class="report-tab-content hidden space-y-4">
                   <div class="bg-white rounded-3xl border border-gray-200 overflow-hidden shadow-sm">
                      <table class="w-full text-left">
                          <thead class="bg-gray-50 border-b border-gray-100">
                              <tr>
                                  <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase">Company / Tradename</th>
                                  <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase">Stall #</th>
                      ...
                      <table class="w-full text-left">
                          <thead class="bg-gray-50 border-b border-gray-100">
                              <tr>
                                  ...
                                  <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase">Submission Date</th>
                      ...
                      <table class="w-full text-left">
                          <thead class="bg-gray-50 border-b border-gray-100">
                              <tr>
                                  ...
                                  <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase text-right">Outstanding Balance</th>
                              </tr>
                          </thead>
  ...
  <!-- BIR Approval Modal -->
  <div id="birApprovalModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] p-4">
    <div class="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-fade-in">
      ...
      <div class="p-8 space-y-6">
        ...
        <div>
          <label for="bir_expiry_input" class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">BIR Registration Expiry Date</label>
          <div class="relative">
          ...
          <p class="text-sm text-gray-500 mt-2">Check the submitted document for the exact expiration date.</p>
        </div>
      </div>
  ...
  <div id="manualBIRUpdateModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[110] p-4">
    <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-fade-in">
      ...
      <form action="admin_dashboard.php" method="POST">
        ...
        <div class="p-8">
          <label for="manual_bir_expiry_input" class="block text-xs font-bold text-gray-700 mb-2 uppercase">New Expiry Date</label>
          ...
          <p class="text-[11px] text-gray-400 mt-2 italic">Updates the record directly without new document submission.</p>
        </div>
  ...
  <div id="individualBillModal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
    <div class="bg-white rounded-[2.5rem] shadow-2xl max-w-lg w-full overflow-hidden animate-slide-up border border-gray-100">
      <!-- Header -->
      <div class="bg-gradient-to-br from-blue-600 to-indigo-700 px-8 py-7 relative overflow-hidden">
        <div class="absolute inset-0 bg-white/10 opacity-20 pointer-events-none"></div>
        <div class="relative z-10">
          <h3 class="text-white font-black text-2xl tracking-tight" id="indiv_modal_tenant">Configure Bill</h3>
          <p class="text-blue-100/80 text-sm font-medium" id="indiv_modal_stall">Set utility amounts and upload receipts</p>
        </div>
      </div>
      ...
      <div class="p-8 space-y-6">
        <!-- Water Bill section -->
        <div class="space-y-3">
          <label class="block text-xs font-black text-gray-400 uppercase tracking-widest px-1">Water Bill</label>
          <div class="flex gap-4 items-center">
            ...
            <label class="flex items-center justify-center w-14 h-14 bg-emerald-100 text-emerald-600 rounded-2xl cursor-pointer hover:bg-emerald-200 transition-all shadow-sm border border-emerald-200">
              <i class="fas fa-camera text-xl"></i>
              <input type="file" id="indiv_water_file" accept="image/*" class="hidden" onchange="previewIndivFile(this, 'water')">
            </label>
          </div>
          <div id="indiv_water_preview" class="hidden px-4 py-2 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded-xl flex items-center gap-2">
        ...
        <div class="space-y-3">
          <label class="block text-xs font-black text-gray-400 uppercase tracking-widest px-1">Electricity Bill</label>
          <div class="flex gap-4 items-center">
            <div class="flex-1 relative">
            ...
            <label class="flex items-center justify-center w-14 h-14 bg-blue-100 text-blue-600 rounded-2xl cursor-pointer hover:bg-blue-200 transition-all shadow-sm border border-blue-200">
              <i class="fas fa-camera text-xl"></i>
              <input type="file" id="indiv_electric_file" accept="image/*" class="hidden" onchange="previewIndivFile(this, 'electric')">
            </label>
          </div>
          <div id="indiv_electric_preview" class="hidden px-4 py-2 bg-blue-50 text-blue-700 text-[10px] font-bold rounded-xl flex items-center gap-2">
            <i class="fas fa-image"></i>
            <span class="truncate">Receipt selected</span>
          </div>
        </div>
      ...
      <div class="px-8 py-6 bg-gray-50 flex gap-4">
        <button onclick="closeIndividualBillModal()" class="flex-1 px-6 py-3.5 bg-white border-2 border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-100 font-bold transition-all">Cancel</button>
        <button onclick="saveIndividualBill()" class="flex-1 px-6 py-3.5 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 font-bold shadow-lg shadow-blue-500/20 transition-all">Confirm</button>
      </div>
    </div>
  </div>
  ...
  <div id="soaGenerationModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden animate-fade-in flex flex-col border border-gray-100">
      <!-- Modal Header -->
      <div class="bg-gradient-to-r from-blue-700 to-indigo-800 px-8 py-6 flex items-center justify-between shadow-lg relative overflow-hidden">
        <div class="absolute inset-0 bg-white/10 opacity-20 pointer-events-none"></div>
        <div class="relative z-10 flex items-center gap-4">
          <div class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/30 shadow-inner">
            <i class="fas fa-file-invoice text-white text-xl"></i>
        ...
        <button type="button" onclick="closeSOAModal()" class="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all border border-white/20 hover:scale-110">
          <i class="fas fa-times"></i>
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-8 py-6 bg-white">
        <form id="soaForm" class="space-y-6">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-black text-gray-500 uppercase tracking-wider mb-2">Reference Number</label>
              <input type="text" id="soa_ref_no" readonly class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl font-bold text-gray-800">
            </div>
            <div>
              <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Prepared By</label>
              <input type="text" value="<?php echo htmlspecialchars($admin_username); ?>" readonly class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl font-bold text-gray-800">
            </div>
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-black text-gray-500 uppercase tracking-wider mb-2">Tenant Name</label>
              <input type="text" id="soa_tenant_name" readonly class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl font-bold text-gray-800">
            </div>
            <div>
              <label class="block text-xs font-black text-gray-500 uppercase tracking-wider mb-2">Unit/Stall</label>
              <input type="text" id="soa_unit_name" readonly class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl font-bold text-gray-800">
            </div>
          </div>

          <div>
            <label class="block text-xs font-black text-gray-500 uppercase tracking-wider mb-2">Billing Period</label>
            <input type="month" id="soa_billing_month" class="w-full px-4 py-3 bg-white border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 transition-all font-bold text-gray-800" onchange="fetchBillingData()">
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
                <p class="text-[10px] text-blue-600 font-black uppercase tracking-widest mb-1">Monthly Rent</p>
              ...
              <div class="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100">
                <p class="text-[10px] text-emerald-600 font-black uppercase tracking-widest mb-1">Water Bill</p>
              ...
              <div class="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                <p class="text-[10px] text-indigo-600 font-black uppercase tracking-widest mb-1">Electric Bill</p>
              ...
              <div class="bg-rose-50/50 p-4 rounded-2xl border border-rose-100">
                <p class="text-[10px] text-rose-600 font-black uppercase tracking-widest mb-1">Arrears</p>
              ...
              <div class="bg-amber-50/50 p-4 rounded-2xl border border-amber-100">
                <p class="text-[10px] text-amber-600 font-black uppercase tracking-widest mb-1">Penalty / Late Fee</p>
              ...
              <div class="bg-purple-50/50 p-4 rounded-2xl border border-purple-100">
                <p class="text-[10px] text-purple-600 font-black uppercase tracking-widest mb-1">Other Charges</p>
            ...
            <div class="bg-gradient-to-r from-gray-900 to-gray-800 p-6 rounded-2xl shadow-xl">
              <div class="flex items-center justify-between text-white">
                <div>
                  <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Total Amount Due</p>
      ...
      <div class="px-8 py-6 bg-gray-50 flex gap-4">
        <button onclick="closeSOAModal()" class="flex-1 px-6 py-4 bg-white border-2 border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-100 font-bold transition-all uppercase tracking-widest text-xs">
          Cancel
        </button>
        <button onclick="generateSOAPrint()" class="flex-1 px-6 py-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 font-bold shadow-xl shadow-blue-500/20 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-2">
  ...
  <div id="notifyBalanceModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in border border-gray-100">
      <!-- Modal Header -->
      <div class="bg-gradient-to-r from-blue-600 to-blue-800 px-8 py-6 flex items-center justify-between shadow-lg">
        <div class="flex items-center gap-4">
          <div class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/30 shadow-inner">
        ...
        <button type="button" onclick="closeNotifyBalanceModal()" class="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all border border-white/20">
          <i class="fas fa-times"></i>
        </button>
      </div>
      ...
      <div class="px-8 py-6 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
            <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Remaining Months</p>
        ...
        <!-- Total Balance -->
        <div class="bg-gradient-to-r from-gray-900 to-gray-800 p-5 rounded-2xl shadow-xl flex items-center justify-between text-white">
          <div>
            <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Estimated Total Balance</p>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between px-1">
            <label class="block text-xs font-black text-gray-500 uppercase tracking-wider">Custom Message (Optional)</label>
            <span class="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg uppercase tracking-wide">Quick Templates</span>
          </div>
          
          <!-- Preset Message Chips -->
          <div class="flex flex-wrap gap-2">
            <button type="button" onclick="setNBPreset('Hi! This is a friendly reminder regarding your outstanding balance at XentroMall. Kindly visit the admin office to settle. Thank you!')"
                    class="px-3 py-1.5 bg-gray-50 hover:bg-emerald-50 text-gray-600 hover:text-emerald-700 border border-gray-100 hover:border-emerald-200 rounded-xl text-[11px] font-bold transition-all transform hover:-translate-y-0.5 active:scale-95">
              Friendly Reminder
            </button>
            <button type="button" onclick="setNBPreset('URGENT NOTICE: Your payment is currently overdue. Please prioritize settling your balance to avoid penalties or interruption. Contact admin for assistance.')"
                    class="px-3 py-1.5 bg-gray-50 hover:bg-rose-50 text-gray-600 hover:text-rose-700 border border-gray-100 hover:border-rose-200 rounded-xl text-[11px] font-bold transition-all transform hover:-translate-y-0.5 active:scale-95">
              Urgent Notice
            </button>
            <button type="button" onclick="setNBPreset('Payment Reminder: Your monthly rent/utility payment is now due. Please settle through the tenant portal or at the admin office. Thank you!')"
                    class="px-3 py-1.5 bg-gray-50 hover:bg-blue-50 text-gray-600 hover:text-blue-700 border border-gray-100 hover:border-blue-200 rounded-xl text-[11px] font-bold transition-all transform hover:-translate-y-0.5 active:scale-95">
              Monthly Due
            </button>
          </div>

          <textarea id="nb_custom_message" rows="3" class="w-full px-4 py-3 bg-white border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 transition-all text-sm text-gray-700" placeholder="Type a message for the tenant..."></textarea>
        </div>
      </div>

      <!-- Actions -->
      <div class="px-8 py-6 bg-gray-50 flex gap-4">
        <button onclick="closeNotifyBalanceModal()" class="flex-1 px-6 py-4 bg-white border-2 border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-100 font-bold transition-all uppercase tracking-widest text-xs">
          Cancel
        </button>
        <button id="btnSendNotifyBalance" onclick="sendPaymentBalanceNotification()" class="flex-1 px-6 py-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 font-bold shadow-xl shadow-blue-500/20 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-2">
  ...
  <script>
    ...
    function setNBPreset(message) {
      document.getElementById('nb_custom_message').value = message;
      showNotification('Template applied!', 'info');
    }
