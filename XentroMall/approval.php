...
if (isset($_POST['submit'])) { 
    ...
    if ($type === 'application') {
        ...
        // If approved, create default contract dates (1 year from today)
        // NOTE: Contract creation should NOT depend on email - it must always run
        if ($status === 'approved') {
            // Try to find tenant by email first, then by user_id
            $tenantRecord = null;
            ...
            // Fallback: find tenant by matching user email from users table
            if (!$tenantRecord && $userId) {
                ...
                if ($userEmail) {
                    $stmtTenant2 = $pdo->prepare('SELECT id FROM tenants WHERE email = :email');
                    $stmtTenant2->execute(['email' => $userEmail]);
                    $tenantRecord = $stmtTenant2->fetch();
                }
            }
            
            // Fallback: Create tenant record if none exists
            if (!$tenantRecord && $userId) {
                try {
                    ...
                    error_log("Created new tenant record ID: $tenantId for user ID: $userId");
                    $tenantRecord = ['id' => $tenantId];
                } catch (PDOException $e) {
                    error_log("Failed to create tenant record: " . $e->getMessage());
                }
            }
            ...
            if ($tenantRecord) {
                ...
                if (!$stmtCheck->fetch()) {
                    // Create default 1-year contract starting today
                    ...
                    error_log("✅ Contract created for tenant ID: $tenantId ($startDate to $endDate)");
                } else {
                    error_log("Contract already exists for tenant ID: $tenantId");
                }
            } else {
                error_log("❌ Could not find or create tenant record for application #$id");
            }
        }
        ...
        if ($status === 'approved') {
            ...
            <head>
                <style>
                    ...
                    .header { background: #10b981; color: white; padding: 40px 30px; text-align: center; border-radius: 12px 12px 0 0; }
    ...
    // Initialize SMS variables so they're always defined
    $smsResult = null;
    $tenantMobile = null;

    // === SMS SENDING (Independent of Email) ===
    error_log("=== SMS SENDING START === Type: $type, ID: $id");
    try {
        // Get tenant mobile number from tenant_details
        if ($type === 'application') {
            $stmtMobile = $pdo->prepare("SELECT mobile FROM tenant_details WHERE id = ?");
            $stmtMobile->execute([$id]);
        } else {
            // For payments, get mobile via user_id
            $stmtMobile = $pdo->prepare("SELECT td.mobile FROM tenant_details td JOIN payments p ON p.user_id = td.user_id WHERE p.id = ?");
            $stmtMobile->execute([$id]);
        }
        ...
        if ($tenantMobile) {
            ...
            if ($status === 'approved') {
                $smsMessage = "XENTROMALL: Congratulations! Your application #$id has been APPROVED. Welcome to XentroMall! Please check your email for details.";
            } else {
            ...
            if ($smsResult['success']) {
                error_log("✅ SMS sent successfully to $tenantMobile for $type #$id");
            } else {
                error_log("❌ SMS failed to send to $tenantMobile: " . $smsResult['response']);
            }
        } else {
            error_log("⚠️ No mobile number found for $type ID: $id");
        }
    } catch (Exception $e) {
        error_log("SMS sending error for $type #$id: " . $e->getMessage());
    }
    error_log("=== SMS SENDING END ===");

    // === EMAIL SENDING (Separate from SMS) ===
    ...
    if (strtolower($status) === 'approved') {
        $message = "✅ Application #$id has been <strong>APPROVED</strong>! Congratulations!";
        ...
        $message = "✅ Application #$id has been <strong>DECLINED</strong>.";
        if ($emailSent && $smsSent) {
            $message .= " Email and SMS notifications sent.";
        } elseif ($emailSent) {
            $message .= " Email notification sent.";
...
<body class="flex items-center justify-center p-4">
        ...
        <div class="glass-card rounded-2xl p-8 max-w-md w-full mx-4 text-center">
            ...
            <div class="mb-6">
                <img src="img/logo.jpg" 
                     alt="Xentro Mall Logo" 
                     class="w-32 h-auto mx-auto mb-4 rounded-lg">
            </div>
            
            <!-- Success Icon -->
            <div class="checkmark inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-emerald-400 to-green-500 rounded-full mb-4">
    ...
    <div class="w-full max-w-md">
        <!-- Back Button -->
        <button onclick="window.location.href='admin_dashboard.php'" class="btn-back text-white px-6 py-3 rounded-xl mb-6 flex items-center gap-2 font-semibold">
        ...
        <div class="glass-card rounded-2xl p-6 shadow-2xl">
            ...
            <div class="text-center mb-6">
                <div class="inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-full mb-3">
            ...
            <form action="approval.php" method="post" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        required
                        class="input-field w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none"
                    ></textarea>
                </div>
