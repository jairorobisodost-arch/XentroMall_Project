            ...
            try {
              ...
              $mail->Body = "
                                <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
                                    <div style='background: #6366f1; background: linear-gradient(135deg, #6366f1 0%, #a855f7 50%, #ec4899 100%); padding: 40px 30px; text-align: center; border-radius: 12px 12px 0 0;'>
    ...
    <main id="mainContent" class="flex-1 flex flex-col gap-6 overflow-auto scrollbar-custom" style="max-height: 90vh;">
          ...
          <div class="grid grid-cols-1 gap-3">
            <?php foreach ($allTenants as $index => $tenant): ?>
              ...
              <div class="gradient-card rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                ...
                <div
                  class="p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between cursor-pointer hover:bg-gray-50 transition-colors gap-4"
                        ...
                        <?php if ($tenant['lease_start_date'] && $tenant['lease_expiration_date']): ?>
                          <div class="space-y-2">
                            <div class="flex justify-between items-center">
                              <span class="text-sm text-purple-700"><i class="fas fa-calendar-check mr-1"></i>Start
                                Date</span>
                              <span
                          ...
                          <div class="ml-4">
                            <div class="text-sm font-medium text-gray-900">
                              ...
                              <?php if (empty($app['admin_viewed'])): ?>
                                <span class="ml-2 unread-notif-badge" id="unread-badge-<?php echo $app['id']; ?>"
                                  title="New Application">
                                  <span class="unread-dot"></span> NEW
                                  <?php if ($app['approved_stalls_count'] > 0): ?>
                                    <span class="ml-1 text-[0.6rem] border-l border-emerald-300 pl-1">ADD'L</span>
                                  <?php endif; ?>
                                </span>
                              <?php endif; ?>
                            </div>
                    ...
                    <?php if ($renewal['payment_proof']): ?>
                      <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                        <div class="flex items-center justify-between">
                          ...
                          <a href="<?php echo htmlspecialchars($renewal['payment_proof']); ?>" target="_blank"
                  ...
                  <tbody class="divide-y divide-gray-200">
                    <?php foreach ($unifiedRenewals as $renewal): ?>
                      <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                          <button onclick="viewDocuments(<?php echo htmlspecialchars(json_encode($renewal)); ?>)"
                            class="text-blue-600 hover:text-blue-700 font-medium text-sm">
      ...
      <section id="paymentsSection" class="hidden space-y-6 animate-fade-in">
        ...
        <div class="flex items-center gap-4 mb-6 border-b border-gray-200 p-1">
          <button onclick="switchPaymentsTab('submissions')" id="tab-payments-submissions"
          ...
          <button onclick="switchPaymentsTab('unpaid')" id="tab-payments-unpaid"
            class="px-6 py-3 text-sm font-medium transition-all border-b-2 border-transparent text-gray-500 hover:text-gray-700">
            <i class="fas fa-exclamation-triangle mr-2 text-orange-500"></i>Unpaid Reminders
            (<?php echo count($unpaidHistory); ?>)
          </button>
        </div>
        ...
        <div id="paymentsUnpaidView" class="hidden space-y-6">
          <div class="mb-4">
            ...
            <p class="text-gray-600">Send notifications to tenants with outstanding balances for the current
              period.</p>
          </div>
          ...
          <?php else: ?>
            <div class="grid grid-cols-1 gap-4">
              <?php foreach ($unpaidHistory as $tenant): ?>
                <div
                  class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all flex flex-col md:flex-row items-center justify-between gap-6">
                  <div class="flex items-center gap-4 flex-1 w-full">
                    <div
                      class="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-blue-600 text-white flex items-center justify-center font-bold text-2xl shadow-lg shrink-0">
                      <?php echo strtoupper(substr($tenant['tradename'] ?? 'T', 0, 1)); ?>
                    </div>
                    <div class="flex-1 min-w-0">
                      <h4 class="font-bold text-xl text-gray-900 truncate">
                        <?php echo htmlspecialchars($tenant['tradename'] ?? 'N/A'); ?>
                      </h4>
                      <div class="flex flex-wrap gap-y-2 gap-x-6 mt-2">
                        <span class="text-sm text-gray-600 flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-lg"><i
                            class="fas fa-store text-gray-400"></i> Stall:
                          <?php echo htmlspecialchars($tenant['stall_number'] ?? 'N/A'); ?></span>
                        <span class="text-sm text-gray-600 flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-lg"><i
                            class="fas fa-peso-sign text-emerald-600 text-xs"></i> Rate:
                          ₱<?php echo number_format($tenant['monthly_rate'] ?? 0, 2); ?></span>
                        <span class="text-sm text-gray-600 flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-lg"><i
                            class="fas fa-phone text-blue-500 text-xs"></i>
                          <?php echo htmlspecialchars($tenant['mobile'] ?? 'N/A'); ?></span>
                      </div>
                    </div>
                  </div>
                          ...
                          <tr class="group hover:bg-blue-50/30 transition-colors">
                            ...
                            <td class="px-4 py-5">
                              <div class="flex flex-col items-center justify-center">
                                ...
                                <input type="file" name="electric_bill_image[<?php echo $key; ?>]"
                                ...
                                <button type="button"
                                  ...
                                  <span class="text-[9px] font-bold text-gray-500 uppercase">Configure
                                    Bill</span>
                                </button>
              ...
              <div class="max-w-2xl mx-auto space-y-8">
                ...
                <div class="grid grid-cols-1 gap-6">
                  <div class="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 text-center shadow-sm">
                    ...
                    <p class="text-[11px] text-emerald-700 font-black uppercase tracking-widest mb-1">
                      Bill Photos</p>
                ...
                <div
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <p class="text-[11px] font-black text-emerald-900 uppercase tracking-widest">
                        Affected Tenants</p>
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col animate-fade-in">
      ...
      <div class="flex items-center justify-between px-8 py-5 border-b border-gray-100 flex-shrink-0">
        ...
        <div class="flex items-center gap-4">
          ...
          <span class="bg-blue-50 border border-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold"
        ...
        <div class="flex items-center gap-6">
          ...
          <nav class="flex gap-2">
            <button onclick="switchReportTab('sales')"
              class="report-tab-btn active px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="sales">Sales
              History</button>
            <button onclick="switchReportTab('tenants')"
              class="report-tab-btn px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="tenants">Tenants
              List</button>
            <button onclick="switchReportTab('pending')"
              class="report-tab-btn px-4 py-2 rounded-xl text-sm font-bold transition-all" data-tab="pending">Pending
              Applications</button>
  ...
  <div id="birApprovalModal"
    ...
    <div class="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-fade-in">
      ...
      <div class="p-8 space-y-6">
        ...
        <div>
          <label for="bir_expiry_input" class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">BIR
            Registration
            Expiry Date</label>
          <div class="relative">
  ...
  <div id="manualBIRUpdateModal"
    ...
    <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-fade-in">
      ...
      <form action="admin_dashboard.php" method="POST">
        ...
        <div class="p-8">
          <label for="manual_bir_expiry_input" class="block text-xs font-bold text-gray-700 mb-2 uppercase">New Expiry
            Date</label>
      ...
      <div class="flex-1 overflow-y-auto px-8 py-6 bg-white">
        <form id="soaForm" class="space-y-6">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-black text-gray-500 uppercase tracking-wider mb-2">Reference
                Number</label>
            ...
            <div>
              <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Prepared
                By</label>
